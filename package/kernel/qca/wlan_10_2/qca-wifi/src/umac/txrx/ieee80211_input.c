...
{  
        ...
        printk("netlink kernel receive message......\n");  
...
{
        struct netlink_kernel_cfg st_netlink_cfg = {0};
            
        /* register callback function AP_kernel_receive */
...
void AP_SendData(const u_int8_t ac_mac[], int8_t c_rssi)  
{  
...
/* begin: add by chenzejun for mac scan by 2015.12.4 */
        if (subtype == IEEE80211_FC0_SUBTYPE_PROBE_REQ)
        {
            static  uint8_t last_mac[MAC_ADDR_LEN];

            if (0 != memcmp((char *)last_mac, wh->i_addr2, MAC_ADDR_LEN))
            {
                AP_SendData(wh->i_addr2, rs->rs_abs_rssi);
                /*
                if (net_ratelimit())
                    printk("[$$$]this is test, line:%d, rssi:%d, %d, mac:%.2X%.2X%.2X%.2X%.2X%.2X\n", 
                        __LINE__, ni->ni_rssi, rs->rs_abs_rssi,
                        wh->i_addr2[0], wh->i_addr2[1], wh->i_addr2[2],
                        wh->i_addr2[3], wh->i_addr2[4], wh->i_addr2[5]);
                 */
                memcpy((char *)last_mac, wh->i_addr2, MAC_ADDR_LEN);  
            }
        }
/* end: add by chenzejun for mac scan by 2015.12.4 */
