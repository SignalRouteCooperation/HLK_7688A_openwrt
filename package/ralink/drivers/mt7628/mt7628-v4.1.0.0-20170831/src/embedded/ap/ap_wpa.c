            ...
            case Ndis802_11AuthModeWPA2PSK:
				...
				{	 
					...
					RTMPSendWirelessEvent(pAd, IW_GROUP_HS_TIMEOUT_EVENT_FLAG, pEntry->Addr, pEntry->wdev->wdev_idx, 0); 
                                                                ...
                                                                {
                                                                        CHAR message[100] = {0};
                                                                        snprintf(message, 100, "GTK already retried, size:%d, Reason:%d", pAd->MacTab.Size, IW_GROUP_HS_TIMEOUT_EVENT_FLAG);
				...
				{			  
					...
					RTMPSendWirelessEvent(pAd, IW_PAIRWISE_HS_TIMEOUT_EVENT_FLAG, pEntry->Addr, pEntry->apidx, 0);
                                                                ...
                                                                {
                                                                        CHAR message[100] = {0};
                                                                        snprintf(message, 100, "msg 3 retried more than three times, size:%d, Disconnect, Reason:%d", pAd->MacTab.Size, IW_PAIRWISE_HS_TIMEOUT_EVENT_FLAG);
                                                                        AP_SendFaultData(pEntry->Addr, message);
                                                                }
                                                                /* end: add by chenzejun for fault manage by 2017.8.4 */
				...
				else if (pEntry->ReTryCounter > (PEER_MSG1_RETRY_TIMER_CTR + 3))
				{
					...
					RTMPSendWirelessEvent(pAd, IW_PAIRWISE_HS_TIMEOUT_EVENT_FLAG, pEntry->Addr, pEntry->wdev->wdev_idx, 0);
                                                                ...
                                                                {
                                                                        CHAR message[100] = {0};
                                                                        snprintf(message, 100, "msg 1 retried more than three times, Disconnect, size:%d, Reason:%d", pAd->MacTab.Size, IW_PAIRWISE_HS_TIMEOUT_EVENT_FLAG);
                                                                        AP_SendFaultData(pEntry->Addr, message);
                                                                }
                                                                /* end: add by chenzejun for fault manage by 2017.8.4 */
