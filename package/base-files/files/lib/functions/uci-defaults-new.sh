...
# gukq 20160707 add for ap lan interface */ 
_ucidef_set_ap_interface() {
	local name=$1
	local iface=$2
	
	echo "name = $name; iface = $iface" >> /root/iface_board 
	
	json_select_object $name
	json_add_string ifname "$iface"
	# [ "$iface" = "${iface%%.*}" ] || json_add_boolean create_vlan 1
	json_add_string force_link "0"
...
# gukq add 20160524 for ap support
ucidef_set_interface_ap_lan() {
	local lan_if="$1"
	
	echo "lanif before = $lan_if" > /root/iface_board 
	
	json_select_object network
	
	# ap wile be written to lan
	_ucidef_set_ap_interface ap "$lan_if"
	json_select ..
}
# added over
...
ucidef_set_led_timer() {
    ...
    json_add_string name $name
    json_add_string sysfs $sysfs
    json_add_string trigger timer
    json_add_string delayon $delayon
    json_add_string delayoff $delayoff
    json_select ..
