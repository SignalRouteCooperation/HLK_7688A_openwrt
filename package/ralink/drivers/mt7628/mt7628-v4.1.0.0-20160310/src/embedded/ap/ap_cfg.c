...
/* begin: add by chenzejun for wifitest by 2015.4.4 */
UCHAR g_cfgdev_debug_flag = 0;
/*
    ==========================================================================
    Description:
    ...
    Example: iwpriv ra0 set DbgSwitch=0
    ==========================================================================
*/
INT Set_CfgDebugSwitch_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
        ULONG dbg;
        ...
        if( dbg <= 5)   
        {
                g_cfgdev_debug_flag = dbg;
                printk("debug_swich=%d\n", g_cfgdev_debug_flag);
        }
...
/* end: add by chenzejun for wifitest by 2015.4.4 */

/* begin: add by chenzejun for apcli verify by 2017.1.30 */
...
/*
    ...
    Return:
        TRUE if all parameters are OK, FALSE otherwise
    Example: iwpriv ra0 set ApcliPassword=12345678 abcdefg
    ==========================================================================
*/
extern 
VOID Set_CfgApcliPassword(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
        UCHAR i, j;
        APCLI_STRUCT *pApCliTab = NULL;
        UINT num = 0;

        if (NULL == arg)    return;


        if (g_cfgdev_debug_flag)  printk("Set_CfgApcliPassword, str:%s\n", arg);


        for (i = 0; i < APCLI_PSK_COUNT; i++)
        {
                pApCliTab = &pAd->ApCfg.ApCliTab[i];

                for (j = 0; j < MAX_APCLI_NUM; j++)
                {
                        if (pApCliTab->stApcliVerify[j].Password[0] == 0)
                        {
                                memcpy(pApCliTab->stApcliVerify[j].Password, arg, strlen(arg));
                                if (g_cfgdev_debug_flag)  printk("Set_CfgApcliPassword, Password[%d]:%s\n", j, pApCliTab->stApcliVerify[i].Password);
                        }
                }
        }
...
#endif /* DATA_QUEUE_RESERVE */
/* begin: add by chenzejun for wifitest add by 2016.2.30 */
	{"DbgSwitch", 	Set_CfgDebugSwitch_Proc},
/* end: add by chenzejun for wifitest add by 2016.2.30 */
/* begin: add by chenzejun for wifitest add by 2016.2.30 */
	{"ApcliPassword", 	Set_CfgApcliPassword},
/* end: add by chenzejun for wifitest add by 2016.2.30 */
	{NULL,}
};
...
/* begin: add by chenzejun for loadbalance by 2017.6.4 */
	else if (bHideSsid == 2 || bHideSsid == 3)
	{       
		//HideSSID=2  mean is  hide
		//HideSSID=3  mean is not hide
		pAd->ApCfg.MBSSID[pObj->ioctl_if].HideSsidMode = bHideSsid;
		return TRUE;
	}
...
/* begin: add by chenzejun for mac filter by 2016.1.30 */
...
/* 
    ...
    Note:
        Usage:
        		1.) iwpriv ra0 add_white_mac  00:1d:fa:56:24:7c  or iwpriv ra0 add_white_mac  00:1d:fa:ff:ff:ff
        		3.) UI needs to prepare at least 4096bytes to get the results
    ==========================================================================
*/
extern 
VOID RTMPIoctlAddWhiteMac(
	IN	PRTMP_ADAPTER	pAd, 
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
        UCHAR i;
        char * pEnd; 
        char * pBegin = wrq->u.data.pointer; 
        UCHAR MacAddr[MAC_ADDR_LEN];


        if ((wrq->u.data.length < 17) || (NULL == wrq->u.data.pointer))    return;

        //printk("add white mac to list, mac:%s\n", wrq->u.data.pointer);

        MacAddr[0] = simple_strtoul(pBegin, &pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[1] = simple_strtoul(pBegin, &pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[2] = simple_strtoul(pBegin, &pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[3] = simple_strtoul(pBegin, &pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[4] = simple_strtoul(pBegin, &pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[5] = simple_strtoul(pBegin, &pEnd, 16);

       
        for (i = 0; i< 64; i++)
        {
                if (pAd->g_white_mac_list[i][0] == 0 && pAd->g_white_mac_list[i][1] == 0 && pAd->g_white_mac_list[i][2] == 0 &&
                    pAd->g_white_mac_list[i][3] == 0 && pAd->g_white_mac_list[i][4] == 0 && pAd->g_white_mac_list[i][5] == 0)
                {

		memcpy(pAd->g_white_mac_list[i], MacAddr, MAC_ADDR_LEN);

                          //printk("add white mac to list, mac-list:%02x:%02x:%02x:%02x:%02x:%02x\n", pAd->g_white_mac_list[i][0], pAd->g_white_mac_list[i][1],
                          //      pAd->g_white_mac_list[i][2], pAd->g_white_mac_list[i][3],pAd->g_white_mac_list[i][4], pAd->g_white_mac_list[i][5]);
...
/* 
    ...
    Note:
        Usage:
        		1.) iwpriv ra0 set_white_mac
        		3.) UI needs to prepare at least 4096bytes to get the results
    ==========================================================================
*/
extern 
VOID RTMPIoctlDelWhiteMac(
	IN	PRTMP_ADAPTER	pAd, 
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
        UCHAR i;
        ...
        if ((wrq->u.data.length < 17) || (NULL == wrq->u.data.pointer))    return;
         ...
         //printk("del white mac to list, mac:%s\n", wrq->u.data.pointer);
        ...
        for (i = 0; i< 64; i++)
        {
                if (pAd->g_white_mac_list[i][0] == MacAddr[0] && pAd->g_white_mac_list[i][1] == MacAddr[1] && 
                    pAd->g_white_mac_list[i][2] == MacAddr[2] && pAd->g_white_mac_list[i][3] == MacAddr[3] &&
                    pAd->g_white_mac_list[i][4] == MacAddr[4] && pAd->g_white_mac_list[i][5] == MacAddr[5])
                {

		memset(pAd->g_white_mac_list[i], 0, MAC_ADDR_LEN);

                          //printk("del white mac to list, mac-list:%02x:%02x:%02x:%02x:%02x:%02x\n", pAd->g_white_mac_list[i][0], pAd->g_white_mac_list[i][1],
                          //      pAd->g_white_mac_list[i][2], pAd->g_white_mac_list[i][3],pAd->g_white_mac_list[i][4], pAd->g_white_mac_list[i][5]);
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	os_alloc_mem(NULL, (UCHAR **)&msg, all_len);
	if (msg == NULL)
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():Alloc memory failed\n", __FUNCTION__));
		return;
	}
	memset(msg, 0, all_len);

	//sprintf(msg+strlen(msg),"\n");
	msg_len = snprintf(&msg[0], all_len, "\nno    white mac list\n");

	for (i=0; i<64; i++)
	{
		if(msg_len + 128  > all_len )
		{
			break;
		}
		
		p_mac = pAd->g_white_mac_list[i];

                        if (p_mac[0] == 0 && p_mac[1] == 0 && p_mac[2] == 0 && p_mac[3] == 0 && p_mac[4] == 0 && p_mac[5] == 0)
                        {
                                continue;
                        }
                        
		msg_len += snprintf(&msg[msg_len], (all_len -msg_len), "%d   %02x:%02x:%02x:%02x:%02x:%02x\n", i, PRINT_MAC(p_mac));
		
                          //printk("get white mac, mac-list:%02x:%02x:%02x:%02x:%02x:%02x\n", pAd->g_white_mac_list[i][0], pAd->g_white_mac_list[i][1],
                          //      pAd->g_white_mac_list[i][2], pAd->g_white_mac_list[i][3],pAd->g_white_mac_list[i][4], pAd->g_white_mac_list[i][5]);
		
	}

	wrq->u.data.length = msg_len;
	//wrq->u.data.length = strlen(msg);
             copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length);
             //printk("msg_len:%d, msg:%s", msg_len, msg);
...
/* end: add by chenzejun for mac filter by 2016.1.30 */

/* begin: add by chenzejun for mac del by 2016.1.30 */
/* 
    ==========================================================================
    Description:
       RTMPIoctlDelMacTableList
	Arguments:
	    pAdapter                    Pointer to our adapter
	    wrq                         Pointer to the ioctl argument
    ...
    Note:
        Usage:
        		1.) iwpriv ra0 del_mac_table 64:a6:51:ee:aa:94
        		3.) UI needs to prepare at least 4096bytes to get the results
    ==========================================================================
*/
VOID RTMPIoctlDelMacTableList(
	IN	PRTMP_ADAPTER	pAd, 
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
        //UCHAR i;
        UCHAR * pEnd; 
        UCHAR * pBegin = wrq->u.data.pointer; 
        USHORT  MacIdx;
        UCHAR MacAddr[6];
        CHAR msg_ok[32] = "delete succes";
        CHAR msg_fail[32] = "no found mac";
        
        if ((wrq->u.data.length < 12) || (NULL == wrq->u.data.pointer))    return;

        //printk("del  mac to list, mac:%s\n", wrq->u.data.pointer);

        MacAddr[0] = simple_strtoul(pBegin, (char **)&pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[1] = simple_strtoul(pBegin, (char **)&pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[2] = simple_strtoul(pBegin, (char **)&pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[3] = simple_strtoul(pBegin, (char **)&pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[4] = simple_strtoul(pBegin, (char **)&pEnd, 16);
        pBegin = pEnd + 1;
        MacAddr[5] = simple_strtoul(pBegin, (char **)&pEnd, 16);
        
        //printk("del mac to list, mac-list:%02x:%02x:%02x:%02x:%02x:%02x\n", MacAddr[0], MacAddr[1], MacAddr[2], MacAddr[3],MacAddr[4], MacAddr[5]);
        ...
        for (MacIdx=0; MacIdx < MAX_LEN_OF_MAC_TABLE; MacIdx++)
        {
                ...
                if (MAC_ADDR_EQUAL(&pAd->MacTab.Content[MacIdx].Addr, MacAddr))
                {
                        MacTableDeleteEntry(pAd, pAd->MacTab.Content[MacIdx].Aid, pAd->MacTab.Content[MacIdx].Addr);

                        //printk("del  mac to list ok\n");
                        NdisReleaseSpinLock(&pAd->MacTabLock);

                        //return msg
                        wrq->u.data.length = sizeof(msg_ok);
                        copy_to_user(wrq->u.data.pointer, msg_ok, wrq->u.data.length);
        ...
        //return msg
        wrq->u.data.length = sizeof(msg_fail);
        copy_to_user(wrq->u.data.pointer, msg_fail, wrq->u.data.length);
     
        return;
}
/* end: add by chenzejun for mac del by 2016.1.30 */
/* begin: add by chenzejun for mac add by 2016.2.30 */
/* 
    ==========================================================================
    Description:
       RTMPIoctlAddMacTableList
	Arguments:
	    pAdapter                    Pointer to our adapter
	    wrq                         Pointer to the ioctl argument
    ...
    Note:
        Usage:
        		1.) iwpriv ra0 add_mac_table 64:a6:51:ee:aa:94
        		3.) UI needs to prepare at least 4096bytes to get the results
    ==========================================================================
*/
VOID RTMPIoctlAddMacTableList(
	IN	PRTMP_ADAPTER	pAd, 
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
        UCHAR * pEnd; 
        ...
        pEntry = MacTableInsertEntry(pAd, MacAddr, &pAd->ApCfg.MBSSID[0].wdev, ENTRY_CLIENT, OPMODE_AP, TRUE);
        if (pEntry)
        {
                pEntry->AuthState = AS_AUTH_OPEN;
                pEntry->Sst = SST_ASSOC; /* what if it already in SST_ASSOC ??????? */
        }
...
/* end: add by chenzejun for mac add by 2016.3.30 */
...
*/
extern 
VOID RTMPIoctlSetApcliPassword(
	IN	PRTMP_ADAPTER	pAd, 
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
        UCHAR i;
        ...
        if (NULL == wrq->u.data.pointer)    return;


        printk("RTMPIoctlSetApcliPassword, str:%s\n", wrq->u.data.pointer);


        for (i = 0; i < MAX_APCLI_NUM; i++)
        {
                pApCliTab = &pAd->ApCfg.ApCliTab[i];
                num = sscanf(pBegin, "%128[^ ] %128[^ ] %128[^ ] %128[^ ] %128[^ ] %128[^ ] %128[^ ] %128[^ ] %128[^ ] %128[^ ]",  
                        pApCliTab->stApcliVerify[0].Password,
                        pApCliTab->stApcliVerify[1].Password,
                        pApCliTab->stApcliVerify[2].Password,  
                        pApCliTab->stApcliVerify[3].Password,  
                        pApCliTab->stApcliVerify[4].Password,  
                        pApCliTab->stApcliVerify[5].Password,  
                        pApCliTab->stApcliVerify[6].Password,  
                        pApCliTab->stApcliVerify[7].Password, 
                        pApCliTab->stApcliVerify[8].Password,  
                        pApCliTab->stApcliVerify[9].Password);
        }
   
        for (i = 0; i< APCLI_PSK_COUNT; i++)
        {
                printk("RTMPIoctlSetApcliPassword, Password[%d]:%s\n", i, pApCliTab->stApcliVerify[i].Password);
        }
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
        ...
        for (i=0; i<APCLI_PSK_COUNT; i++)
        {
                ...
                if (pApCliTab->stApcliVerify[i].Password[0])
                {
                        
                        msg_len += snprintf(&msg[msg_len], (all_len -msg_len), "password:%s result:%d\n",
                                pApCliTab->stApcliVerify[i].Password, pApCliTab->stApcliVerify[i].bVerifyResult);
                }
        }
...
/* end: add by chenzejun for apcli verify by 2017.1.30 */
	...
	IN	ULONG					Data)
{
	...
	switch(cmd)
	{
		...
		case CMD_RTPRIV_IOCTL_STATISTICS:
			...
			break;
/* begin: add by chenzejun for mac del by 2016.1.30 */
		case CMD_RTPRIV_IOCTL_DEL_MAC_TABLE_LIST:
			//RTMPIoctlDelMacTableList(pAd, wrq);
			break;
/* end: add by chenzejun for mac del by 2016.1.30 */
/* begin: add by chenzejun for mac add by 2016.2.30 */
		case CMD_RTPRIV_IOCTL_ADD_MAC_TABLE_LIST:
			//RTMPIoctlAddMacTableList(pAd, wrq);
			break;
/* end: add by chenzejun for mac add by 2016.2.30 */
/* begin: add by chenzejun for mac filter by 2016.1.30 */
		case CMD_RTPRIV_IOCTL_AP_ADD_WHITE_MAC:
			RTMPIoctlAddWhiteMac(pAd, wrq);
			break;
		case CMD_RTPRIV_IOCTL_AP_DEL_WHITE_MAC:
			RTMPIoctlDelWhiteMac(pAd, wrq);
			break;
		case CMD_RTPRIV_IOCTL_AP_GET_WHITE_MAC:
			RTMPIoctlGetWhiteMac(pAd, wrq);
			break;
/* end: add by chenzejun for mac filter by 2016.1.30 */
/* begin: add by chenzejun for apcli verify by 2017.1.30 */
		case CMD_RTPRIV_IOCTL_GET_APCLI_VERIFY_RESULT:
			RTMPIoctlGetVerifyResult(pAd, wrq);
			break;
/* end: add by chenzejun for apcli verify by 2017.1.30 */
