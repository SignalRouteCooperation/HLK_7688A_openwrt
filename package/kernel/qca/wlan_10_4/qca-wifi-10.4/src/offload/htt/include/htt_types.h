...
#define HTT_NBUF_ARRAY_SZ 32
    ...
    adf_nbuf_t t2h_buf_tmp[HTT_NBUF_ARRAY_SZ_PARTNER];
    adf_os_atomic_t t2h_buf_ref_cnt[HTT_NBUF_ARRAY_SZ_PARTNER];
    ...
    struct {
        ...
        char *pool_vaddr;
        u_int32_t pool_paddr;
        adf_os_dma_mem_context(memctx); /** @todo: not sure if this is required. */
    } frag_descs;
    ...
    adf_os_spinlock_t    htt_buf_list_lock;
    ATH_LIST_HEAD(, htt_pdev_buf_entry)  buf_list_head;
    adf_nbuf_queue_t     mgmtbufQ;
    adf_os_spinlock_t    mgmtbufLock;
#if PEER_FLOW_CONTROL
