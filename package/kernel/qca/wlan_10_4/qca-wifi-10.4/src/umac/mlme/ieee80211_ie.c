    ...
    )
{
   ...
   if(widebwie) {
    ...
    if(((ni->ni_vap->iv_des_mode >= IEEE80211_MODE_11AC_VHT20)) && (ni->ni_vap->iv_des_mode <= IEEE80211_MODE_11AC_VHT80_80)) {
        if((phymode < IEEE80211_MODE_11AC_VHT20) || (ni->ni_vap->iv_des_mode < phymode)) {
            IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_DEBUG,
                       "%s : Overriding the PHY MODE %d to desired PHY Mode %d\n", __func__,phymode,ni->ni_vap->iv_des_mode);
            phymode = ni->ni_vap->iv_des_mode;
        }
    } else if(ni->ni_vap->iv_des_mode < phymode) {
        IEEE80211_DPRINTF_IC(ic, IEEE80211_VERBOSE_LOUD, IEEE80211_MSG_DEBUG,
                   "%s : Overriding the PHY MODE %d to desired PHY Mode %d\n", __func__,phymode,ni->ni_vap->iv_des_mode);
        phymode = ni->ni_vap->iv_des_mode;
    }
                     ...
                     struct ieee80211_framing_extractx *extractx)
{
    ...
    vhtcap_info &= (((vap->iv_vhtsubfer) && (tx_streams > 1)) ?  ic->ic_vhtcap : ~IEEE80211_VHTCAP_SU_BFORMER);
                    ...
                    struct ieee80211_framing_extractx *extractx)
{
    ...
    struct ieee80211_channel *adjusted_chan = NULL;
    ...
    if (negotiate_bw == 1) {
        ...
        adjusted_chan  =  ieee80211_find_dot11_channel(ic,
                ic->ic_curchan->ic_ieee, 0, IEEE80211_MODE_11AC_VHT80);

        if (adjusted_chan) {
            vhtop->vht_op_ch_freq_seg1 = adjusted_chan->ic_vhtop_ch_freq_seg1;
        }
            /* Note: This is applicable only for 80+80Mhz mode */
            vhtop->vht_op_ch_freq_seg2 = 0;
    }
    else {
        if ((chwidth == IEEE80211_CWM_WIDTH160) && vap->iv_rev_sig_160w) {
             /* update vht_op_ch_freq_seg1 and vht_op_ch_freq_seg2 for 160*/
            if(IEEE80211_IS_CHAN_11AC_VHT160(ic->ic_curchan)){
                adjusted_chan  =  ieee80211_find_dot11_channel(ic,
                            ic->ic_curchan->ic_ieee, 0, IEEE80211_MODE_11AC_VHT80);
                /* An equivalent 80 MHz channel entry mut be present, else it
                   indicates a serious issue elsewhere. */
                adf_os_assert_always(adjusted_chan != NULL);
                vhtop->vht_op_ch_freq_seg1 =
                    adjusted_chan->ic_vhtop_ch_freq_seg1;
    ...
    if ((!vap->iv_rev_sig_160w) && (extractx != NULL) &&
        ...
        */
        ...
        if (adjusted_chan) {
            ...
            /* We should never enter this, since an 80 MHz channel will be
             * always be available as a replacement for 160/80+80 MHz */
            printk("%s: Warning: Could not find channel with 80 MHz mode for "
                   "IEEE channel %hhu. Investigate!",
                    ...
                    struct ieee80211com *ic,  u_int8_t subtype)
{
    ...
    switch(ic->ic_chanchange_chwidth)
    {
        ...
        case CHWIDTH_VHT80:
            ...
            break;
        case CHWIDTH_VHT160:
            if (IEEE80211_IS_CHAN_11AC_VHT160(ic->ic_chanchange_channel)) {
                new_ch_width = IEEE80211_VHTOP_CHWIDTH_160;
            } else if (IEEE80211_IS_CHAN_11AC_VHT80_80(ic->ic_chanchange_channel)) {
        ...
        default:
            printk("%s: Invalid destination channel width %d specified\n",
                    __func__, ic->ic_chanchange_chwidth);
            adf_os_assert_always(0);
            break;
    }
