...
void HTCFlushRxHoldQueue(HTC_TARGET *target, HTC_ENDPOINT *pEndpoint)
{
    ...
    while (!HTC_QUEUE_EMPTY(&pEndpoint->RxBufferHoldQueue)) {
       /* QueueHead becomes NULL in some scenarios - Added NULL check */
        PDL_LIST pList = &(pEndpoint->RxBufferHoldQueue.QueueHead);
        if(pList == NULL) {
            break;
        }
