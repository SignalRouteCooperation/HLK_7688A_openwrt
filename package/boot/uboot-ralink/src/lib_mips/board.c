...
/* begin: add by chenzejun for tftp by reset key by 2016.1.22 */
            u32 reset_key = RALINK_REG(RT2880_REG_PIODATA_RST);

            #if defined(MT7628_FPGA_BOARD) || defined (MT7628_ASIC_BOARD)
            reset_key = (reset_key>>6)&0x1;       //reset key (jvhua), pin  gpio38, RT2880_REG_PIODATA_RST 6 pin in mt7628
            #elif  defined (MT7620_ASIC_BOARD) || defined (MT7620_FPGA_BOARD)
            reset_key = (reset_key>>12)&0x1;       //reset key (jvhua), pin  gpio12, RT2880_REG_PIODATA_RST 12 pin in mt7620
            #endif

            //reset key is push
            if (reset_key == 0)   BootType = '2';
            //printf("---RALINK_REG1--key:0x%x, 0x%x, line:%d\n", RALINK_REG(RT2880_REG_PIODATA_RST), reset_key, __LINE__);
/* end: add by chenzejun for tftp by reset key by 2016.1.22 */
	...
	if(BootType == '3') {
		...
		case '2':
			if (reset_key == 0)
			{
                                		...
                                		argv[1] = "0x80A00000";
                                                #endif
                                		strncpy(argv[2], "recovery.bin", ARGV_LEN);
			}
			else
			{
