...
#ifdef JOYLINK_SUPPORT

#include <net/sock.h>  
#include <linux/netlink.h> 


extern struct sock *p_nl_sock_unit;
extern UCHAR	g_JOYLINK[];


#define JYLK_POTOCOL_RET 1
#define JYLK_DATA_MAX_LEN   512


typedef struct GNU_PACKED _FRAME_JOYLINK_DATA
{
        UCHAR            JYLK[4];
        UCHAR            Type;
        UCHAR            Length;
...
extern void DEBUG_hexdump(unsigned char Type, unsigned char *buf,unsigned int len);
...
/*
    ...
    ==========================================================================
*/
void JYLK_SendDataToJoyLink(USHORT MsgType, UCHAR *pucMAC, UCHAR *pucData, USHORT DataSize)  
{  
        ...
        pst_tlv_data = (AP_TLV_DATA *)pc_msg_data;
        pst_tlv_data->us_tlv_type = JOYLINK_TLV_TYPE_DATA;
        pst_tlv_data->us_tlv_len =  sizeof(AP_TLV_DATA) + DataSize;
        memcpy((char *)(pst_tlv_data + 1), pucData, DataSize);  
        slen = slen + pst_tlv_data->us_tlv_len; 

        //printk("kernel send message");  
        nlh->nlmsg_len = slen;
        nlh->nlmsg_type = MsgType;   //data of get

        netlink_unicast(p_nl_sock_unit, skb_1, AP_NLMSG_PID_JOYLINK, MSG_DONTWAIT);  
        return;
}  
...
/*
    ...
    ==========================================================================
*/
void JYLK_SendBeaconToJoyLink(USHORT MsgType, UCHAR *pucMAC, UCHAR *pSSID, USHORT SSIDLen, UCHAR *pVendor, USHORT VendorLen)  
{  
        struct sk_buff *skb_1;  
        struct nlmsghdr *nlh;  
        char   *pc_msg_data;  
        ...
        pc_msg_data = pc_msg_data + pst_tlv_data->us_tlv_len;
        pst_tlv_data = (AP_TLV_DATA *)pc_msg_data;
        pst_tlv_data->us_tlv_type = JOYLINK_TLV_TYPE_SSID;
        pst_tlv_data->us_tlv_len =  sizeof(AP_TLV_DATA) + SSIDLen;
        memcpy((char *)(pst_tlv_data + 1), pSSID, SSIDLen); 
        slen = slen + pst_tlv_data->us_tlv_len; 
        ...
        pc_msg_data = pc_msg_data + pst_tlv_data->us_tlv_len;
        pst_tlv_data = (AP_TLV_DATA *)pc_msg_data;
        pst_tlv_data->us_tlv_type = JOYLINK_TLV_TYPE_VENDOR;
        pst_tlv_data->us_tlv_len =  sizeof(AP_TLV_DATA) + VendorLen;
        memcpy((char *)(pst_tlv_data + 1), pVendor, VendorLen); 
        slen = slen + pst_tlv_data->us_tlv_len; 
...
/*
    ...
    ==========================================================================
*/
void JYLK_SendBeaconRawToJoyLink(USHORT MsgType, UCHAR *pucMAC, UCHAR *pVendor, USHORT VendorLen)  
{  
        struct sk_buff *skb_1;  
        struct nlmsghdr *nlh;  
        char   *pc_msg_data;  
...
/*
    ...
    ==========================================================================
*/
static INT JYLK_RecvJoyLinkBeacon(IN PRTMP_ADAPTER pAd, RX_BLK *pRxBlk)
{
        PFRAME_802_11 pHeader = (PFRAME_802_11)pRxBlk->pHeader;
        PEID_STRUCT pEID = NULL;
        UCHAR *pSSID = NULL;
        UCHAR *pVendor = NULL;
        UCHAR *pVendor_raw = NULL;


        /* timestamp interval  10 byte */
        /* capablity 2 byte */

	
        /* rawdata, pEID */
        pEID = (PEID_STRUCT) &pHeader->Octet[12];

        if (0)
        {
        	UCHAR *pData = (CHAR *)pEID;

        	//DEBUG_hexdump(CFGDEV_LOG_RX, pRxBlk->pHeader, pRxBlk->DataSize);
        	
        	if (pHeader->Hdr.Addr2[0] == 0x9c && pHeader->Hdr.Addr2[1] == 0x65)        
        		printk("receive beacon len:%d, data:%02X%02X%02X%02X  || %02X%02X%02X%02X || %02X%02X\n", 
        ...
        while (((UCHAR *)pEID + pEID->Len + 1) < ((UCHAR *)pHeader + pRxBlk->DataSize))
        {
                switch(pEID->Eid)
                {
                        case IE_SSID:
                        {
                                if ((pEID->Len == MAX_LEN_OF_SSID) && (0 == NdisCmpMemory(pEID->Octet, "JYLK", 4)))
                                {
                                        pSSID = pEID->Octet;
                                }
                                break;
                        ...
                        case IE_VENDOR_SPECIFIC:
                        {
                                ...
                                if (0 == NdisCmpMemory(pEID->Octet, "JYLK", 4))
                                {
                                        pVendor_raw = pEID->Octet;
                                }
                                break;
                        }
                        default:
        ...
        if (pSSID && pVendor)
        {
                CFGDEV_DBG_PRINTK(CFGDEV_LOG_RX, "[kernel] ap receive joylink beacon, mac:%02X%02X%02X%02X%02X%02X, Sequence:%d, SSID:%.*s, len:%d, data:%02X%02X%02X%02X %02X%02X%02X%02X\n", 
                        pHeader->Hdr.Addr2[0], pHeader->Hdr.Addr2[1], pHeader->Hdr.Addr2[2], pHeader->Hdr.Addr2[3], pHeader->Hdr.Addr2[4], pHeader->Hdr.Addr2[5],
                        pHeader->Hdr.Sequence, 18, pSSID, pRxBlk->DataSize,
                        pVendor[0], pVendor[1], pVendor[2], pVendor[3], pVendor[4], pVendor[5], pVendor[6], pVendor[7]);

                (VOID)JYLK_SendBeaconToJoyLink(AP_JOYLINK_TYPE_BEACON, pHeader->Hdr.Addr2, pSSID, MAX_LEN_OF_SSID, pVendor, pEID->Len);
                return JYLK_POTOCOL_RET;
        }

        if (pVendor_raw)
        {
                CFGDEV_DBG_PRINTK(CFGDEV_LOG_RX, "[kernel] ap receive joylink beacon raw, mac:%02X%02X%02X%02X%02X%02X, Sequence:%d, len:%d, data:%02X%02X%02X%02X %02X%02X%02X%02X\n", 
                        pHeader->Hdr.Addr2[0], pHeader->Hdr.Addr2[1], pHeader->Hdr.Addr2[2], pHeader->Hdr.Addr2[3], pHeader->Hdr.Addr2[4], pHeader->Hdr.Addr2[5],
                        pHeader->Hdr.Sequence,  pRxBlk->DataSize,
                        pVendor_raw[0], pVendor_raw[1], pVendor_raw[2], pVendor_raw[3], pVendor_raw[4], pVendor_raw[5], pVendor_raw[6], pVendor_raw[7]);

                (VOID)JYLK_SendBeaconRawToJoyLink(AP_JOYLINK_TYPE_BEACON_RAW, pHeader->Hdr.Addr2, pVendor_raw, pEID->Len);
                return JYLK_POTOCOL_RET;
        }
...
/*
    ...
    ==========================================================================
*/
static INT JYLK_RecvJoyLinkData(IN PRTMP_ADAPTER pAd, RX_BLK *pRxBlk)
{
        PFRAME_802_11 pHeader = (PFRAME_802_11)pRxBlk->pHeader;
        FRAME_JOYLINK_DATA *pJoyData = NULL;
        UCHAR Offset = 6;    /*80211mac:24byte   snap:6byte, type 2*/
        UCHAR *pucData = NULL;
        USHORT DataLen = 0;
        ...
        if (0)
        {
                static UINT Count = 5000;
        	UCHAR *pData = (CHAR *)&pHeader->Octet[Offset + 2];

        	if (Count)  DEBUG_hexdump(CFGDEV_LOG_RX, pData, pRxBlk->DataSize - sizeof(HEADER_802_11) - Offset - 2);

                Count--;
        	
        	if (pHeader->Hdr.Addr2[0] == 0x9c && pHeader->Hdr.Addr2[1] == 0x65)        
        		printk("receive data len:%d, data:%02X%02X%02X%02X  || %02X%02X%02X%02X || %02X%02X\n", 
        		sizeof(HEADER_802_11), 
        		pData[0], pData[1], pData[2], pData[3], pData[4], pData[5], pData[6], pData[7], 
        		pData[8], pData[9]);
        }

        //rawdata position, llc
        /*80211mac:24byte   snap:6byte, type 2*/
        pucData = (UCHAR *)&pHeader->Octet[Offset];
        pJoyData = (FRAME_JOYLINK_DATA *)&pHeader->Octet[Offset + 2];
        DataLen = pRxBlk->DataSize - sizeof(HEADER_802_11) - Offset - 2;

        if (DataLen > JYLK_DATA_MAX_LEN)  
        {
                //CFGDEV_DBG_PRINTK(CFGDEV_LOG_RX, "[kernel] ap receive data frame error, len = %d", pRxBlk->DataSize);
                return 0;
        }
        

        //pucData[0] and  pucData[1] is LLC type
        if ((0 == NdisCmpMemory(pucData, g_JOYLINK, 2)) && (0 == NdisCmpMemory(pJoyData->JYLK, "JYLK", 4)))
        {
                pucData = (UCHAR *)(pJoyData + 1);
                CFGDEV_DBG_PRINTK(CFGDEV_LOG_RX, "[kernel] ap receive joylink data, mac:%02X%02X%02X%02X%02X%02X, Sequence:%d, type:%d, len:%d, data:%02X%02X%02X%02X%02X%02X%02X%02X\n", 
                        pHeader->Hdr.Addr2[0], pHeader->Hdr.Addr2[1], pHeader->Hdr.Addr2[2], pHeader->Hdr.Addr2[3], pHeader->Hdr.Addr2[4], pHeader->Hdr.Addr2[5],
                        pHeader->Hdr.Sequence, pJoyData->Type, pJoyData->Length,
                        pucData[0], pucData[1], pucData[2], pucData[3], pucData[4], pucData[5], pucData[6], pucData[7]);

                (VOID)JYLK_SendDataToJoyLink(AP_JOYLINK_TYPE_DATA, pHeader->Hdr.Addr2, (UCHAR *)pJoyData, DataLen);
        
                return JYLK_POTOCOL_RET;
        }

        if ((0 == NdisCmpMemory(pucData, TestType, 2)) && (0 == NdisCmpMemory(pJoyData->JYLK, "JYLK", 4)))
        {
                pucData = (UCHAR *)(pJoyData + 1);
                CFGDEV_DBG_PRINTK(CFGDEV_LOG_RX, "[kernel] ap receive test data, mac:%02X%02X%02X%02X%02X%02X, Sequence:%d, type:%d, len:%d, data:%02X%02X%02X%02X%02X%02X%02X%02X\n", 
                        pHeader->Hdr.Addr2[0], pHeader->Hdr.Addr2[1], pHeader->Hdr.Addr2[2], pHeader->Hdr.Addr2[3], pHeader->Hdr.Addr2[4], pHeader->Hdr.Addr2[5],
                        pHeader->Hdr.Sequence, pJoyData->Type, pJoyData->Length,
                        pucData[0], pucData[1], pucData[2], pucData[3], pucData[4], pucData[5], pucData[6], pucData[7]);
...
#endif
/* end: add by chenzejun for joylink by 2015.4.4 */
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		if ((pHeader->FC.SubType == SUBTYPE_BEACON) && (ADHOC_ON(pAd)) && pEntry)
		{
			Update_Rssi_Sample(pAd, &pEntry->RssiSample, &pRxBlk->rx_signal, pRxBlk->rx_rate.field.MODE, pRxBlk->rx_rate.field.BW);
             ...
             /* begin: add by chenzejun for joylink by 2015.4.4 */
#ifdef JOYLINK_SUPPORT
	if (pHeader->FC.SubType == SUBTYPE_BEACON)
             {
                        if (JYLK_POTOCOL_RET == JYLK_RecvJoyLinkBeacon(pAd, pRxBlk)) 
                        {
                                goto done;
                        }
             }
...
BOOLEAN rtmp_rx_done_handle(RTMP_ADAPTER *pAd)
{
	...
	while (1)
	{
		...
		*/
        ...
        {
        //rxblk.pHeader->FC.Type == FC_TYPE_DATA
             JYLK_RecvJoyLinkData(pAd, pRxBlk);
