...
ol_tx_enqueue_pflow_ctrl(ol_txrx_vdev_handle vdev, adf_nbuf_t nbuf, uint32_t peer_id, uint8_t tid, uint8_t nodrop, uint8_t hol)
{
     ...
     */
    if ((!nodrop && (adf_nbuf_queue_len(&peer->tidq[tid].nbufq) > pdev->pflow_ctl_queue_max_len[peer_id][tid])) ||
    ...
    if((pdev->pflow_ctrl_mode == HTT_TX_MODE_PUSH_NO_CLASSIFY) ||
...
#else
        {
            ...
            if ( ( ftype == CB_FTYPE_MCAST2UCAST)
                ...
                status = ol_tx_ll_fast(vdev, &netbuf, 1, HTT_INVALID_PEER, HTT_TX_EXT_TID_INVALID);
            } else {
                status = ol_tx_ll_cachedhdr(vdev, netbuf, HTT_INVALID_PEER, HTT_TX_EXT_TID_INVALID);
            }
