...
extern void AP_Netlink_init(RTMP_ADAPTER *pAd);
...
NDIS_STATUS APInitialize(RTMP_ADAPTER *pAd)
{
 ...
 /* begin: add by chenzejun for mac filter by 2016.1.30 */
             ...
             pAd->g_white_mac_list[0][0] = 0x00;
             pAd->g_white_mac_list[0][1] = 0x1d;
             pAd->g_white_mac_list[0][2] = 0xfa;
             pAd->g_white_mac_list[0][3] = 0xff;
             pAd->g_white_mac_list[0][4] = 0xff;
             pAd->g_white_mac_list[0][5] = 0xff;
	...
	for (MacIdx=0; MacIdx < MAX_LEN_OF_MAC_TABLE; MacIdx++)
	{
		...
		if (drop == TRUE) {
			...
			if (pEntry->Sst == SST_ASSOC)
			{
				...
				{
					CHAR message[100] = {0};
					snprintf(message, 100, "Send DISASSOC, size:%d, Reason:%d", pAd->MacTab.Size, Reason);
					AP_SendFaultData(pAd->MacTab.Content[MacIdx].Addr, message);
				}
				/* end: add by chenzejun for fault manage by 2017.8.4 */
    ...
    if((pEntry->AuthMode == Ndis802_11AuthModeWPA) || (pEntry->AuthMode == Ndis802_11AuthModeWPA2) || (pAd->ApCfg.MBSSID[apidx].wdev.IEEE8021X == TRUE))
	{
		...
		if (pEntry)
		{
			...
			if ((pMbss->HotSpotCtrl.HotSpotEnable == 1) && (pMbss->wdev.AuthMode == Ndis802_11AuthModeWPA2) && (pEntry->hs_info.ppsmo_exist == 1))
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("event eapol start, %x:%x:%x:%x\n",
