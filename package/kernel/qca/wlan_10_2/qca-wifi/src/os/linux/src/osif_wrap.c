...
/* proc.zhu */
static int wc_proc_open(struct inode *i, struct file *f)
{
...
//    proc_entry = create_proc_entry(wc->wc_proc_name, WRAP_PROC_PERM, NULL);
    const struct file_operations wc_proc_fops = {
	 owner:THIS_MODULE,
	 open:wc_proc_open,
	 read:seq_read,
	 llseek:seq_lseek,
    ...
    proc_entry = proc_create(wc->wc_proc_name, WRAP_PROC_PERM, NULL, &wc_proc_fops);
