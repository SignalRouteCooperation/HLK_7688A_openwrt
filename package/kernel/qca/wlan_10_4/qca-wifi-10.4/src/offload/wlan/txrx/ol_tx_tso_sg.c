...
ol_tx_tso_sg_desc_display(void *tx_desc)
{
    ...
    adf_os_print("==========TSO SG Descriptor==============>\n");
    adf_os_print("tx_tso_sg_desc %x \n",(A_UINT32 )tx_tso_sg_desc);
    adf_os_print("HOST_TSO_SG::l2_length %x\n",tx_tso_sg_desc->l2_length);
    adf_os_print("HOST_TSO_SG::l3_hdr_length %x\n",tx_tso_sg_desc->l3_hdr_size);
    adf_os_print("HOST_TSO_SG::l2_l3_l4_hdr_size %x\n",tx_tso_sg_desc->l2_l3_l4_hdr_size);
    adf_os_print("HOST_TSO_SG::last_segment %x\n",tx_tso_sg_desc->last_segment);
    adf_os_print("HOST_TSO_SG::data_len %x\n",tx_tso_sg_desc->data_len);
    adf_os_print("HOST_TSO_SG::segment_count %x\n",tx_tso_sg_desc->segment_count);

    for (count = 0; count < tx_tso_sg_desc->segment_count; count++)
        adf_os_print("seg addr[%d]: %x, seg_len: %d\n", count, tx_tso_sg_desc->seg_paddr_lo[count], tx_tso_sg_desc->seg_len[count]);
}
...
ol_tx_msdu_desc_display(void *msdu_desc)
{
    ...
    adf_os_print("HOST_TSO_SG::tso_enable %x\n",msdu_link_ext_p->tso_enable);
    adf_os_print("HOST_TSO_SG::tcp_flag %x\n",msdu_link_ext_p->tcp_flag);
    adf_os_print("HOST_TSO_SG::tcp_flag_mask %x\n",msdu_link_ext_p->tcp_flag_mask);
    adf_os_print("HOST_TSO_SG::l2_length %x\n",msdu_link_ext_p->l2_length);
    adf_os_print("HOST_TSO_SG::ip_length %x\n",msdu_link_ext_p->ip_length);
    adf_os_print("HOST_TSO_SG::tcp_seq_number %x\n",msdu_link_ext_p->tcp_seq_number);
    adf_os_print("HOST_TSO_SG::ip_identification %x\n",msdu_link_ext_p->ip_identification);
    adf_os_print("HOST_TSO_SG::ipv4_checksum_en %x\n",msdu_link_ext_p->ipv4_checksum_en);
    adf_os_print("HOST_TSO_SG::udp_over_ipv4_checksum_en %x\n",msdu_link_ext_p->udp_over_ipv4_checksum_en);
    adf_os_print("HOST_TSO_SG::udp_over_ipv6_checksum_en %x\n",msdu_link_ext_p->udp_over_ipv6_checksum_en);
    adf_os_print("HOST_TSO_SG::tcp_over_ipv4_checksum_en %x\n",msdu_link_ext_p->tcp_over_ipv4_checksum_en);
    adf_os_print("HOST_TSO_SG::tcp_over_ipv6_checksum_en %x\n",msdu_link_ext_p->tcp_over_ipv6_checksum_en);
    adf_os_print("HOST_TSO_SG::buf0_ptr_31_0 %x\n",msdu_link_ext_p->buf0_ptr_31_0);
    adf_os_print("HOST_TSO_SG::buf0_len %x\n",msdu_link_ext_p->buf0_len);
    adf_os_print("HOST_TSO_SG::buf1_ptr_31_0 %x\n",msdu_link_ext_p->buf1_ptr_31_0);
    adf_os_print("HOST_TSO_SG::buf1_len %x\n",msdu_link_ext_p->buf1_len);
    adf_os_print("HOST_TSO_SG::buf2_ptr_31_0 %x\n",msdu_link_ext_p->buf2_ptr_31_0);
    adf_os_print("HOST_TSO_SG::buf2_len %x\n",msdu_link_ext_p->buf2_len);
    adf_os_print("HOST_TSO_SG::buf3_ptr_31_0 %x\n",msdu_link_ext_p->buf3_ptr_31_0);
    adf_os_print("HOST_TSO_SG::buf3_len %x\n",msdu_link_ext_p->buf3_len);
    adf_os_print("HOST_TSO_SG::buf4_ptr_31_0 %x\n",msdu_link_ext_p->buf4_ptr_31_0);
    adf_os_print("HOST_TSO_SG::buf4_len %x\n",msdu_link_ext_p->buf4_len);
    adf_os_print("HOST_TSO_SG::buf5_ptr_31_0 %x\n",msdu_link_ext_p->buf5_ptr_31_0);
    adf_os_print("HOST_TSO_SG::buf5_len %x\n",msdu_link_ext_p->buf5_len);
    ...
    ol_txrx_vdev_handle vdev)
{
    ...
    adf_os_print("++++++++++ TSO SG STATISTICS +++++++++++\n");
    adf_os_print("TSO SG TX Pkts       : %u\n", stats->tx.tso.tso_pkts.pkts);
    adf_os_print("TSO SG TX Bytes      : %u\n", stats->tx.tso.tso_pkts.bytes);
    adf_os_print("Non-TSO SG TX Pkts   : %u\n", stats->tx.tso.non_tso_pkts.pkts);
    adf_os_print("Non-TSO SG TX Bytes  : %u\n", stats->tx.tso.non_tso_pkts.bytes);
    adf_os_print("TSO SG Dropped Pkts  : %u\n", stats->tx.tso.tso_dropped.pkts);
    adf_os_print("TSO SG Dropped Bytes : %u\n", stats->tx.tso.tso_dropped.bytes);
    adf_os_print("TSO SG Desc Total %d In Use : %d\n", pdev->tx_tso_desc.pool_size, \
                                                 stats->tx.tso.tso_desc_cnt);
}
...
ol_tx_rst_tso_stats(ol_txrx_vdev_handle vdev)
{
    ...
    adf_os_print(".....Resetting TSO SG Stats \n");
    /* Tx */
...
#if TSO_SG_DBG
    adf_os_print("Seq No. %u \n",msdu_link_ext_p->tcp_seq_number);
#endif

#if TSO_SG_DBG
    adf_os_print("Freeing TX TSo Descriptor %x\n",sw_tso_sg_desc);
#endif
...
ol_tx_tso_sg_sw_desc_alloc_fill(adf_nbuf_t msdu,
                             struct ol_txrx_pdev_t *pdev)
{
    ...
    if (adf_os_unlikely(!sw_tso_sg_desc)) {
        adf_os_print("HOST_TSO_SG:Dropping Pkt->SW TSO Descriptor allocation \
                      failed \n");
	adf_nbuf_free(msdu);
        return NULL;
    }
    ...
    if (adf_nbuf_tso_tcp_v4(msdu))  {
        iphdr = (adf_net_iphdr_t *)(adf_nbuf_network_header(msdu));
        sw_tso_sg_desc->ipv4_id = adf_os_ntohs(iphdr->ip_id);
        sw_tso_sg_desc->ip_hdr_type = TSO_IPV4;
        sw_tso_sg_desc->l3_hdr_size = ((A_UINT8 *)tcphdr - (A_UINT8 *)iphdr);
        sw_tso_sg_desc->l2_length = ((A_UINT8 *)iphdr - adf_nbuf_data(msdu));
    } else if (adf_nbuf_tso_tcp_v6(msdu)) {
      /* For IPV6 */
        ...
        sw_tso_sg_desc->l2_length = ((A_UINT8 *)ip6hdr - adf_nbuf_data(msdu));
    }

    sw_tso_sg_desc->mss_size = adf_nbuf_tcp_tso_size(msdu);
    ...
    sw_tso_sg_desc->tcp_seq = adf_nbuf_tcp_seq(msdu);

    /* tcphdr->doff is number of 4-byte words in tcp header */
    sw_tso_sg_desc->l2_l3_l4_hdr_size = adf_nbuf_l2l3l4_hdr_len(msdu);  /* Number of bytes to payload start */

    if(A_STATUS_FAILED == adf_nbuf_map_nbytes_single(pdev->osdev, msdu,
                                                     ADF_OS_DMA_TO_DEVICE,
						     adf_nbuf_headlen(msdu))) {
        adf_os_print("HOST_TSO_SG:Dropping Pkt, failed to map fragment buffer of msdu \n");
	ol_tx_tso_sg_desc_free(pdev, sw_tso_sg_desc);
	adf_nbuf_free(msdu);
	return NULL;
    }

    sw_tso_sg_desc->seg_paddr_lo[0] = adf_nbuf_get_frag_paddr_lo(msdu, 0);
...
inline int
ol_tx_tso_sg_process_skb(ol_txrx_vdev_handle vdev,adf_nbuf_t msdu)
{
    ...
    total_payload_len = adf_nbuf_len(msdu) - sw_tso_sg_desc->l2_l3_l4_hdr_size;
    mss_size = sw_tso_sg_desc->mss_size;
    segment_space = sw_tso_sg_desc->mss_size;
    adf_nbuf_set_fctx_type(msdu, (u_int32_t)sw_tso_sg_desc, CB_FTYPE_TSO_SG);

    do {
        if (!skb_data_verified && (adf_nbuf_headlen(msdu) > sw_tso_sg_desc->l2_l3_l4_hdr_size)) {
             ...
             */
            fragment_len = adf_nbuf_headlen(msdu) - sw_tso_sg_desc->l2_l3_l4_hdr_size;
             ...
             */
            fragment_len = adf_nbuf_get_frag_size(msdu, cur_frag);
            if (A_STATUS_FAILED == adf_nbuf_frag_map(pdev->osdev,
                                                     ...
                                                     cur_frag)) {
                adf_os_print("HOST_TSO_SG:Dropping Pkt, failed to map fragment buffer of msdu \n");
                ol_tx_tso_sg_desc_free(pdev, (struct ol_tx_tso_desc_t *)adf_nbuf_get_fctx(msdu));
                adf_nbuf_free(msdu);
                return -EIO;
                }
            fragment_paddr = adf_nbuf_get_frag_paddr_0(msdu);
            total_payload_len -= fragment_len;
            cur_frag++;
        }
        ...
        do {
             ...
             */
                ...
                if (!sw_tso_sg_desc->last_segment) {
                    ...
                    if(!(seg_msdu = adf_nbuf_clone(msdu))) {
                    ...
                    adf_nbuf_set_fctx_type(seg_msdu, (u_int32_t)seg_tso_sg_desc, CB_FTYPE_TSO_SG);
