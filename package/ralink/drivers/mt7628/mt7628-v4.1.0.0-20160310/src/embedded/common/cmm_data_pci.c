	...
	UCHAR RxRingNo)
{
	...
	if ((pRxD->LS0 == 0) || ((pRxD->LS0 == 1) && (pAd->DropInvalidPacket >= 1)))
	{
		...
		if (pAd->DropInvalidPacket >= 1)
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Ring No = %d\n", RxRingNo));
...
//fixed the issue that landi auto link ssid dont discover the ssid
#if 1 //#ifdef AUTOLINK_SSID_SUPPORT
{
