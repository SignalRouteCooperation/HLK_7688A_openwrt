...
extern void ieee80211_vap_iter_get_chan(void *arg, wlan_if_t vap);
...
static void ieee80211_connection_state_connecting_entry(void *ctx)
{
    ...
    if (ieee80211_ic_enh_ind_rpt_is_set(sm->vap_handle->iv_ic)) {
        ...
        if (!sm->vap_handle->iv_ic->ic_mpsta_vap || (sm->vap_handle->iv_ic->ic_mpsta_vap && sm->vap_handle->iv_mpsta)) {
            ...
            wlan_iterate_vap_list(sm->vap_handle->iv_ic, ieee80211_vap_iter_get_chan, &chan);
            ...
            if (phychan && (sm->vap_handle->iv_ic->ic_curchan != phychan)) {
                if (!ic->ic_is_mode_offload(ic)) {
                    ...
                    /* In Enh_Independent mode ,do instant channel change */
                    ic->ic_prevchan = ic->ic_curchan;
                    ic->ic_curchan = phychan;
                    ...
                    TAILQ_FOREACH(tmp_vap, &ic->ic_vaps, iv_next) {
                        if(tmp_vap->iv_opmode == IEEE80211_M_HOSTAP ||
                            ...
                            if (ieee80211_vap_ready_is_set(tmp_vap)){
                                tmp_vap->iv_bsschan = phychan;
                                tmp_vap->iv_des_chan[tmp_vap->iv_des_mode] = phychan;
