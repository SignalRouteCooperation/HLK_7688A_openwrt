...
IMG_PREFIX:=$(CURDATE)-$(LAST_COMMIT)-$(KTVERSION)-$(PROJECT)-$(if $(CONFIG_VERSION_FILENAMES),$(VERSION_NUMBER)-)$(BOARD)$(if $(SUBTARGET),-$(SUBTARGET))
...
define Device/Build/check_size
	@[ $$(($(subst k,* 1024,$(subst m, * 1024k,$(1))))) -gt "$$(stat -c%s $@)" ] || { \
		echo "WARNING: Image file $@ is too big === $(1) = $$(stat -c%s $@) ===" >&2; \
		#rm -f $@; \
	}
endef
