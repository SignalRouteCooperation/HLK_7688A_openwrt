...
/* gukq 20160603 add macro to define random device */
// using this macro instead of static string "/dev/random"
...
int random_pool_ready(void)
{
	 ...
	 * completely.
	 */
	fd = open(DEV_RANDOM, O_RDONLY | O_NONBLOCK);
	...
	if (res < 0) {
		wpa_printf(MSG_ERROR, "random: Cannot read from %s: "
			   "%s", DEV_RANDOM, strerror(errno));
		res = 0;
	}
	...
	wpa_printf(MSG_INFO, "random: Only %u/%u bytes of strong "
		   ...
		   (unsigned) dummy_key_avail, (unsigned) sizeof(dummy_key), DEV_RANDOM);
...
static void random_read_fd(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "random: Got %u/%u bytes from %s",
		   (unsigned) res,
		   (unsigned) (sizeof(dummy_key) - dummy_key_avail), DEV_RANDOM);
	dummy_key_avail += res;
...
void random_init(const char *entropy_file)
{
	...
	random_fd = open(DEV_RANDOM, O_RDONLY | O_NONBLOCK);
	if (random_fd < 0) {
#ifndef CONFIG_NO_STDOUT_DEBUG
		int error = errno;
		perror("open(/dev/urandom)");
	...
	wpa_printf(MSG_DEBUG, "random: Trying to read entropy from %s",
		   DEV_RANDOM);
