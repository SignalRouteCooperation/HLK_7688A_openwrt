...
void AP_SendConnectData(const UCHAR ac_mac[], UCHAR uc_type_wifi, UCHAR uc_signal, USHORT ui_count)  
{  
...
void AP_SendFaultData(const UCHAR ac_mac[], unsigned char *puc_info)  
{  
        struct sk_buff *skb_1;  
        struct nlmsghdr *nlh;  
        char   *pc_msg_data;  
        ...
        printk("mac:%02X%02X%02X%02X%02X%02X, info:%s \n", ac_mac[0], ac_mac[1], ac_mac[2], ac_mac[3], ac_mac[4], ac_mac[5], puc_info);  
        ...
        //tlv: AP_TLV_TYPE_FAULT_INFO
        pc_msg_data = pc_msg_data + pst_tlv_data->us_tlv_len;
        pst_tlv_data = (AP_TLV_DATA *)pc_msg_data;
        pst_tlv_data->us_tlv_type = AP_TLV_TYPE_MESSAGE;
        info_len = strlen(puc_info) + 1;  //'\0'
        pst_tlv_data->us_tlv_len =  sizeof(AP_TLV_DATA) + info_len;
        memcpy((char *)(pst_tlv_data + 1), puc_info, info_len);  
        slen = slen + pst_tlv_data->us_tlv_len; 


        //printk("kernel send message");  
        nlh->nlmsg_len = slen;
        nlh->nlmsg_type = AP_MSG_TYPE_FAULT_INFO;   //fault
			...
			)
			{
					...
					(wdev->IEEE8021X == TRUE))
				{
					...
					pEntry->WpaState = AS_AUTHENTICATION;
                                                ...
                                                /* begin: add by chenzejun for mac scan by 2016.1.22 */
                                                        AP_SendConnectData(pEntry->Addr, pAd->phy_ctrl.rf_band_cap, pAd->CommonCfg.TxPowerPercentage, pAd->MacTab.Size);
                                                /* end: add by chenzejun for mac scan by 2016.1.22 */


                                        /* begin: add by chenzejun for mac filter by 2016.1.30 */
                                        #if 0 //#ifdef AUTOLINK_SSID_SUPPORT				
                                        if ((pEntry->AuthMode >= Ndis802_11AuthModeWPA) && (pAd->ApCfg.MBSSID[pEntry->func_tb_idx].bAutoLinkEnable == TRUE))
                                        {
                                                int i = 0;

                                                    pEntry->mac_white_flag = 0;
                                                    //001d76-00301c:001dfa:Fujian LANDI Commercial Equipment Co.,Ltd

                                                    for (i = 0; i < 64; i++)
                                                   {
                                                        if (pEntry->Addr[0] == pAd->g_white_mac_list[i][0] && pEntry->Addr[1] == pAd->g_white_mac_list[i][1]  && pEntry->Addr[2] == pAd->g_white_mac_list[i][2]  && 
                                                            (pEntry->Addr[3] == pAd->g_white_mac_list[i][3] ||0xff == pAd->g_white_mac_list[i][3]) && 
                                                            (pEntry->Addr[4] == pAd->g_white_mac_list[i][4] ||0xff == pAd->g_white_mac_list[i][4]) && 
                                                            (pEntry->Addr[5] == pAd->g_white_mac_list[i][5] ||0xff == pAd->g_white_mac_list[i][5]))
                                                        {
                                                            pEntry->mac_white_flag = 1;
                                                            break;
                                                        }

                                                     }

                                                    if (pEntry->mac_white_flag != 1)
                                                    {
                                                	        //retval = RT_CfgSetWPAPSKKey(pAd, landi_pmk_passphrases, 8, (PUCHAR)pMBSSStruct->Ssid, pMBSSStruct->SsidLen, pEntry->mac_PMK);
                                                            pEntry->mac_black_flag = 1;
                                                            pEntry->mac_PMK[0] = 0xee; 
                                                    }
                                                    else
                                                    {
                                                           pEntry->mac_black_flag = 0;
                                                    }
                                                    
                                                    if (g_cfgdev_debug_flag == 1) 
                                                    {
                                                                printk("ASSOCATE, line:%d, mac_white_flag:%d, apidx:%d, ssid:%s, MAC:%02x%02x%02x%02x%02x%02x\n", 
                                                                        __LINE__,
                                                                        pEntry->mac_white_flag, pEntry->func_tb_idx, pAd->ApCfg.MBSSID[pEntry->func_tb_idx].Ssid, 
...
{
        CHAR message[100] = {0};
        snprintf(message, 100, "peer disassoc, size:%d, Reason:%d", pAd->MacTab.Size, Reason);
        AP_SendFaultData(Addr2, message);
}
#endif
/* end: add by chenzejun for fault manage by 2017.8.4 */
...
{
        CHAR message[100] = {0};
        snprintf(message, 100, "MlmeDisassoc, size:%d", pAd->MacTab.Size);
        AP_SendFaultData(DisassocReq->Addr, message);
}
/* end: add by chenzejun for fault manage by 2017.8.4 */
