...
#define  htt_ffcache_update_desc_set_peer_id(_ffcache, _peerid) \
        _ffcache[5] = adf_os_cpu_to_le32(_peerid)
...
#define  htt_ffcache_update_msduavglen(_pdev, _ffcache, _len) \
	   _ffcache[4] = adf_os_cpu_to_le32(_len) ;
...
#define HTT_TX_PDEV_LOCK(x) adf_os_spin_lock_bh(x)
#define HTT_TX_PDEV_UNLOCK(x) adf_os_spin_unlock_bh(x)
#else
