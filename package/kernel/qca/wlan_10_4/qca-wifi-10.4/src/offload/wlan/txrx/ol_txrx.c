...
#define STATS_LENGTH (4 * 1024)
                {
                    ...
                    if(!stats_wqp) {
                        htt_t2h_stats_print(stats_info_list, req->base.print.concise, scn->target_type);
                        break;
                    }
                    ...
                    *(uint32_t *)(stats_buffer + 4) = (uint32_t)scn;
                    adf_os_create_work(pdev->osdev, stats_wqp,
                            stats_deferred_work, stats_buffer);
                    adf_os_mem_copy(((uint8_t *)stats_buffer + 8), stats_info_list, length);
