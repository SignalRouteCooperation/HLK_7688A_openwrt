...
#undef DEFAULT_MIN_SCAN_DWELL_TIME
    ...
    case IEEE80211_CONFIG_ASSOC_DENIAL_NOTIFICATION:
        {
           if (wlan_get_acl_policy(vap) != IEEE80211_MACCMD_POLICY_OPEN){
...
#if ATH_PERF_PWR_OFFLOAD
    printk("Configuring cts2self protection for DTIM beacon : %s \n",enable?"enable":"disable");
    /* Update in F/W as well. */
    if(ic->ic_is_mode_offload(ic))
            ol_ath_set_vap_cts2self_prot_dtim_bcn(vap);
#else
    printk("Configuring cts2self protection for DTIM beacon not supported on this platform \n");
#endif
    return 0;
