...
{  
        ...
        {  
                ...
                {
                        printk("7628 kernel  received [pid:0x%x] message:%s\n", AP_NLMSG_PID_MACSCAN, str) ; 
                }
...
void AP_Netlink_init(void)
{
        ...
        /*?迆?迆o?∩∩?“辰??? netlink socket㏒?2⊿℅⊿?¯車谷 kernel_recieve() o‘那y?車那?那y?Y */
        ...
        printk("7628 kernel mscan init......\n");  

        //?a?????????迆???∩∩∩?“米??那足a
        if (p_nl_sock == NULL)
        {
                p_nl_sock = netlink_kernel_create(&init_net, NETLINK_TEST, &st_netlink_cfg);
                if(!p_nl_sock)  
                {  
                        printk("7628 can not create a netlink socket, type:%d\n", NETLINK_TEST);  
                        return;  
                } 
        }
