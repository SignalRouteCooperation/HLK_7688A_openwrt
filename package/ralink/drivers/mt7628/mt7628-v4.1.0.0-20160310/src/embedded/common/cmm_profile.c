...
/*
    ...
    ==========================================================================
*/
void JYLK_SendSsidPwdToJoyLink(UCHAR *pucSSID, USHORT SsidLen, UCHAR *pucPasswd, USHORT PasswdLen)  
{  
        ...
        memcpy((char *)(pst_tlv_data + 1), pucSSID, SsidLen);  
        ...
        pst_tlv_data = (AP_TLV_DATA *)pc_msg_data;
        pst_tlv_data->us_tlv_type = JOYLINK_TLV_TYPE_PASSWD;
        pst_tlv_data->us_tlv_len =  sizeof(AP_TLV_DATA) + PasswdLen;
        memcpy((char *)(pst_tlv_data + 1), pucPasswd, PasswdLen);  
        slen = slen + pst_tlv_data->us_tlv_len; 

        //printk("kernel send message");  
        nlh->nlmsg_len = slen;
        nlh->nlmsg_type = AP_JOYLINK_TYPE_INFO_UPDATE;   //data of get
	...
	IN RTMP_STRING *buffer)
{
	...
	/*ApCliVerifyEnable*/
	if(RTMPGetKeyParameter("ApCliVerifyEnable", tmpbuf, 128, buffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
		{
	                          //printk("===> ApCliVerifyEnable, %d\n", __LINE__);
			pApCliEntry = &pAd->ApCfg.ApCliTab[i];
			if ((strncmp(macptr, "0", 1) == 0))
			{
				pApCliEntry->bApCliVerifyEnable = FALSE;
                                                   pApCliEntry->PMKCurrentId = 0xff;
				pApCliEntry->PMKNextId = 0;
				
			}
			else if ((strncmp(macptr, "0", 1) == 1))
			{
				pApCliEntry->bApCliVerifyEnable = TRUE;
				pApCliEntry->PMKCurrentId = 0xff;
				pApCliEntry->PMKNextId = 0;
			}

                                       /// init 
                                      memset(pApCliEntry->stApcliVerify, 0, sizeof(pApCliEntry->stApcliVerify));
			for (j = 0; j < APCLI_PSK_COUNT; j++)
			{
			        pApCliEntry->stApcliVerify[j].bVerifyResult = 0xff;
			        memset(pApCliEntry->stApcliVerify[j].Password, 0, sizeof(pApCliEntry->stApcliVerify[j].Password));
			        memset(pApCliEntry->stApcliVerify[j].PMK, 0, sizeof(pApCliEntry->stApcliVerify[j].PMK));
			}
		}
	}
	/*ApCliVerifyKey1*/
	if(RTMPGetKeyParameter("ApCliVerifyKey1", tmpbuf, 128, buffer, TRUE))
	{
                          for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
		{
			int KeyId = 0;
			...
			NdisMoveMemory(pApCliEntry->stApcliVerify[KeyId].Password, macptr, strlen(macptr) + 1);
			//pApCliEntry->PSKLen = strlen(macptr);
	                          //printk("===> ApCliVerifyKey1, %d\n", __LINE__);

			(VOID)RT_CfgSetWPAPSKKey(pAd, pApCliEntry->stApcliVerify[KeyId].Password, strlen(pApCliEntry->stApcliVerify[KeyId].Password),
				                        (PUCHAR)pApCliEntry->CfgSsid, (INT)pApCliEntry->CfgSsidLen, pApCliEntry->stApcliVerify[KeyId].PMK);

		}

	}
	/*ApCliVerifyKey2*/
	if(RTMPGetKeyParameter("ApCliVerifyKey2", tmpbuf, 128, buffer, TRUE))
	{
                          for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
		{
			int KeyId = 1;
	...
	/*ApCliVerifyKey3*/
	if(RTMPGetKeyParameter("ApCliVerifyKey3", tmpbuf, 128, buffer, TRUE))
	{
                          for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
		{
			int KeyId = 2;
	...
	/*ApCliVerifyKey4*/
	if(RTMPGetKeyParameter("ApCliVerifyKey4", tmpbuf, 128, buffer, TRUE))
	{
                          for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
		{
			int KeyId = 3;
	...
	/*ApCliVerifyKey5*/
	if(RTMPGetKeyParameter("ApCliVerifyKey5", tmpbuf, 128, buffer, TRUE))
	{
                          for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
		{
			int KeyId = 4;
...
static int rtmp_parse_wpapsk_buffer_from_file(RTMP_ADAPTER *pAd, RTMP_STRING *buffer, INT BSSIdx)
{
	...
	if (ret == FALSE)
		return FALSE;
                ...
                //printk("joylink modify psk\n");
                JYLK_SendSsidPwdToJoyLink(pAd->ApCfg.MBSSID[i].Ssid, pAd->ApCfg.MBSSID[i].SsidLen, tmpbuf, len);
                #endif
					...
					if(RTMPGetKeyParameter(tok_str, tmpbuf, 33, pBuffer, FALSE))
						{
							...
							if (bSSIDxIsUsed == FALSE)
							{
								bSSIDxIsUsed = TRUE;
                                                                ...
                                                                /* begin: add by chenzejun for joylink config by 2016.4.20 */
                                                                        #ifdef JOYLINK_SUPPORT
                                                                        JYLK_SendSsidPwdToJoyLink(pAd->ApCfg.MBSSID[i].Ssid, pAd->ApCfg.MBSSID[i].SsidLen, pAd->ApCfg.MBSSID[i].WpaPsk, pAd->ApCfg.MBSSID[i].WpaPskLen);
                                                                        #endif
                                                                /* end: add by chenzejun for joylink config by 2016.4.20 */
...
/* begin: add by chenzejun for mac filter by 2016.1.30 */
	if(RTMPGetKeyParameter("AutoLinkEnable", tmpbuf, 32, pBuffer, TRUE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); macptr; macptr = rstrtok(NULL,";"), i++)
	    {
			int apidx = i;
			...
			if(simple_strtol(macptr, 0, 10) != 0)  /*Enable*/
			{
				pAd->ApCfg.MBSSID[apidx].bAutoLinkEnable = TRUE;

			}
			else /*Disable*/
			{
				pAd->ApCfg.MBSSID[apidx].bAutoLinkEnable = FALSE;
			}
