...
#if TCP_CHECKSUM_OFFLOAD
	if(adf_nbuf_get_tx_cksum(netbuf)==HTT_TX_L4_CKSUM_OFFLOAD) {
...
#if MESH_MODE_SUPPORT
        if (extfree) {
            adf_nbuf_set_next(msdu, msdus_ext);
            msdus_ext = msdu;
        } else
#endif
        {
    ...
    int had_error)
{
    ...
    if (tx_desc->pkt_type == ol_tx_frm_tso) {
        ...
        struct ol_tx_me_buf_t *buf = (struct ol_tx_me_buf_t *)
            adf_nbuf_get_fctx(tx_desc->netbuf);
        ...
        adf_os_mem_unmap_nbytes_single(pdev->osdev, paddr, ADF_OS_DMA_TO_DEVICE, len);
