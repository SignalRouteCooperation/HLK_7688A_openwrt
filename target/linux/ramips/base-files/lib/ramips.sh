...
ramips_board_detect() {
	...
	*)
	...
	[ -f $check_ban ] && echo "ban""$RAMIPS_BOARD_TYPE" > /tmp/sysinfo/board_type
	# end zhangzf

	# 20170109 add landi,yunjin oem support
	check_landi='/usr/lib/opkg/info/landi-robot.control'
	[ -f $check_landi ] && echo "LD$RAMIPS_BOARD_TYPE" > /tmp/sysinfo/board_type

	check_yunjin='/usr/lib/opkg/info/yunjin-oem-files.control'
	[ -f $check_yunjin ] && echo "YJ$RAMIPS_BOARD_TYPE" > /tmp/sysinfo/board_type
	# end zhangzf
