...
htt_h2t_frag_desc_bank_cfg_msg(struct htt_pdev_t *pdev)
{
    ...
    *msg_word = 0;
    ...
    if (pdev->nss_wifiol_ctx) {
        osif_ol_nss_htc_send(pdev->nss_wifiol_ctx, pdev->nss_wifiol_id, msg, pdev->htc_endpoint);
    } else
#endif
    {
    ...
    u_int64_t cookie)
{
    ...
    /* set the length of the message */
    if (adf_nbuf_put_tail(msg, HTT_H2T_STATS_REQ_MSG_SZ) == NULL) {
