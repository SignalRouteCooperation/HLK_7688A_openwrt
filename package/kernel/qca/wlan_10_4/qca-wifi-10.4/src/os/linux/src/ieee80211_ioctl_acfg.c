            ...
            a_uint8_t *buffer, a_int32_t Length)
{
    ...
    if (ptr->flags & ACFG_RTS_DISABLED) {
        ...
        if (IEEE80211_RTS_MIN <= ptr->val && ptr->val <= IEEE80211_RTS_MAX)
        {
            val = ptr->val;
        } else {
            return -EINVAL;
        }
        return ieee80211_ucfg_set_rts(vap, val);
    }
    return 0;
}
              ...
              a_uint8_t *buffer, a_int32_t Length)
{
	...
	if (ptr->flags & ACFG_TXPOW_FIXED) {
		retv = ieee80211_ucfg_set_txpow(vap, ptr->val);
	} else {
...
static void
acfg_get_sta_iter_len(void *ptr, wlan_node_t node)
{
    ((acfg_sta_info_req_t *)ptr)->len += sizeof(acfg_sta_info_t);
}

static void
acfg_get_sta_info(void *ptr, wlan_node_t node)
{
    acfg_sta_info_t *si = ((acfg_sta_info_req_t *)ptr)->info;
    a_uint8_t *macaddr = wlan_node_getmacaddr(node);
    ieee80211_rate_info rinfo;
    wlan_rssi_info rssi_info;
    wlan_chan_t chan;

    si->associd = wlan_node_get_associd(node);
    chan = wlan_node_get_chan(node);
    si->channel = chan->ic_ieee;
    if (wlan_node_txrate_info(node, &rinfo) == 0) {
        si->txrate = RATE_KBPS_TO_MBPS(rinfo.rate);
    }
    if (wlan_node_getrssi(node, &rssi_info, WLAN_RSSI_RX) == 0) {
        si->rssi = rssi_info.avg_rssi;
    }
    si->inact = wlan_node_get_inact(node);
    wlan_node_get_rxseqs(node, (u_int16_t *)&si->rxseq, sizeof(si->rxseq));
    wlan_node_get_txseqs(node, (u_int16_t *)&si->txseq, sizeof(si->txseq));
    si->cap = wlan_node_getcapinfo(node);
    si->athcap = wlan_node_get_ath_flags(node);
    si->htcap = wlan_node_get_htcap(node);
    si->state = wlan_node_get_state_flag(node);
    si->erp = wlan_node_get_erp(node);
    IEEE80211_ADDR_COPY(si->macaddr, macaddr);

    ((acfg_sta_info_req_t *)ptr)->info = (acfg_sta_info_t*)((a_uint8_t *)si + sizeof(acfg_sta_info_t));
}
 ...
 bad:
    ...
    if (num_global_scn && invalid_vap) {
       ...
       ic = (struct ieee80211com *)sc->sc_ieee;
        ...
        ic = (struct ieee80211com *)ol_global_scn[0];
    }
