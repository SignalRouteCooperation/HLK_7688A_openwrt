        ...
        caddr_t *param)
{
    ...
    if (error) {
        ...
        if (wlan_acs_start_scan_report(vap, 1, IEEE80211_MAX_DWELL_ACS_REPORT, request.max_dwell_time) != EOK) {
        ...
        if (wlan_acs_start_scan_report(vap, 1, IEEE80211_MIN_DWELL_ACS_REPORT, request.min_dwell_time) != EOK) {
        ...
        if (wlan_acs_start_scan_report(vap, 1, IEEE80211_SCAN_REST_TIME, request.rest_time) != EOK) {
        ...
        if (wlan_acs_start_scan_report(vap, 1, IEEE80211_SCAN_MODE, request.scan_mode) != EOK) {
