...
USHORT g_debug_init = 0;
struct sock *p_nl_sock = NULL;
/* end: add by chenzejun for mac scan by 2016.1.22 */

/* begin: add by chenzejun for attendence-client by 2017.6.4 */
// connect info node
typedef struct  MAC_CONN_NODE_INFO
{
        unsigned char  ac_mac[MAC_ADDR_LEN];     /* mac address */
        
        unsigned long  ul_first_unixtime;   //first unix time
        unsigned long  ul_last_unixtime;   //last unix time

       u_int8_t new_bssid[IEEE80211_ADDR_LEN];


       //unsigned long  ul_last_unixtime_local;   //last unix time
       u_int8_t new_ssid[IEEE80211_NWID_LEN + 1];
       u_int8_t new_ssid_len;

}MAC_CONN_NODE_INFO;

MAC_CONN_NODE_INFO st_mac_node[36] = {0};
int g_curr_node_id = 0;
unsigned long  g_ul_startup_unixtime = 0;   //first unix time
//#define AP_MSG_TYPE_REPONSE_TIME   4
//#define AP_MSG_TYPE_CLEAR_BUF   5



static void AP_clear_list(void)  
{
        memset(&st_mac_node, 0, sizeof(st_mac_node));
        g_curr_node_id = 0;
        return;
}

static MAC_CONN_NODE_INFO *AP_query_list(unsigned char *puc_mac)  
{
        int i = 0;

        if (puc_mac == NULL)    return  NULL;

        
        for(i = 0; i < ARRAY_SIZE(st_mac_node); i++)
        {
                if (0 == memcmp(puc_mac, st_mac_node[i].ac_mac, sizeof(st_mac_node[i].ac_mac)))
                {
                        return &st_mac_node[i];
                }
        }
...
static void AP_update_list(unsigned char *puc_msg)  
{
        AP_TLV_DATA *pst_tlv_data = NULL;
        unsigned char *puc_mac =  NULL;
        unsigned long *pul_firsttime =  NULL;
        unsigned long *pul_lasttime =  NULL;
        MAC_CONN_NODE_INFO *pst_node = NULL;
        
        if (puc_msg == NULL)    return;

        //tlv mac
        pst_tlv_data =  (AP_TLV_DATA *)puc_msg;
        if (pst_tlv_data->us_tlv_type != AP_TLV_TYPE_MAC)       return;
        puc_mac =  (unsigned char *)(pst_tlv_data + 1);

        //tlv first time
        pst_tlv_data =  (AP_TLV_DATA *)((char *)pst_tlv_data + pst_tlv_data->us_tlv_len);
        if (pst_tlv_data->us_tlv_type != AP_TLV_TYPE_TIME)       return;
        pul_firsttime = (unsigned long *)(pst_tlv_data + 1);

        //tlv last time
        pst_tlv_data =  (AP_TLV_DATA *)((char *)pst_tlv_data + pst_tlv_data->us_tlv_len);
        if (pst_tlv_data->us_tlv_type != AP_TLV_TYPE_TIME)       return;
        pul_lasttime = (unsigned long *)(pst_tlv_data + 1);
        

        pst_node =  AP_query_list(puc_mac);
        if (pst_node == NULL)
        {
                pst_node = &st_mac_node[g_curr_node_id];
                memcpy((char *)pst_node->ac_mac, puc_mac, MAC_ADDR_LEN);  
        
                //next id update
                g_curr_node_id++;
                if (g_curr_node_id >=ARRAY_SIZE(st_mac_node))  g_curr_node_id = 0;
                
                //printk("new--");
        }

        pst_node->ul_first_unixtime = *pul_firsttime;
        pst_node->ul_last_unixtime = *pul_lasttime;
        //pst_node->new_ssid_len = snprintf(pst_node->new_ssid, IEEE80211_NWID_LEN, "%ld-%ld", pst_node->ul_first_unixtime, pst_node->ul_last_unixtime);
        

        /*
        printk("update_list mac:%02x%02x%02x-%02x%02x%02x, %ld, %ld\n",  
                puc_mac[0],  puc_mac[1],  puc_mac[2], 
                puc_mac[3],  puc_mac[4],  puc_mac[5],
                pst_node->ul_first_unixtime, pst_node->ul_last_unixtime);
        */
        return;
}
static void AP_update_time(unsigned char *pc_bssid, MAC_CONN_NODE_INFO *pst_node)  
{
        unsigned long  time_sec = htonl(get_seconds());
        unsigned char *pc_time = (unsigned char *)&time_sec;


        pst_node->new_bssid[0] = pc_bssid[4];
        pst_node->new_bssid[1] = pc_bssid[5];
        pst_node->new_bssid[2] = pc_time[0];
        pst_node->new_bssid[3] = pc_time[1];
        pst_node->new_bssid[4] = pc_time[2];
        pst_node->new_bssid[5] = pc_time[3];

        if (pst_node->new_ssid_len == 0)
        {
                pst_node->new_ssid_len= snprintf(pst_node->new_ssid, IEEE80211_NWID_LEN, "%02x:%02x:%02x:%02x:%02x:%02x-%ld", 
                        pc_bssid[0], pc_bssid[1], pc_bssid[2], pc_bssid[3], pc_bssid[4], pc_bssid[5],
                        pst_node->ul_first_unixtime);
        }
}


void AP_SendKeepLiveData(void)  
{  
        ...
        nlh->nlmsg_len = slen;
        nlh->nlmsg_type = AP_MSG_TYPE_KEEP_ALIVE;   //data of scannig
...
/* end: add by chenzejun for attendence-client by 2017.6.4 */
...
#if 0
static void AP_Netlink_Release(void)  
{  
        if(p_nl_sock)  
        {  
...
{  
        ...
        {  
                ...
                if (nlh->nlmsg_type == AP_MSG_TYPE_HELLO)
                {
                        printk("netlink kernel [id:%d] received [pid:0x%x] message:%s\n", NETLINK_24G, nlh->nlmsg_pid, (char *)NLMSG_DATA(nlh)); 
                }
                else if (nlh->nlmsg_type == AP_MSG_TYPE_REPONSE_TIME)
                {
                        AP_update_list(NLMSG_DATA(nlh));
                }
                else if (nlh->nlmsg_type == AP_MSG_TYPE_CLEAR_BUF)
                {
                        AP_clear_list();
                }
                else if (nlh->nlmsg_type == AP_MSG_TYPE_KEEP_ALIVE)
                {
                        AP_SendKeepLiveData();
                }
        } 
...
void AP_Netlink_init(void)
{
        ...
        printk("netlink kernel mscan init......\n");  
        ...
        if (p_nl_sock == NULL)
        {
                ...
                if(!p_nl_sock)  
                {  
                        printk("netlink kernel can not create a netlink socket, type:%d\n", NETLINK_24G);  
                        return;  
                } 
        }
...
void AP_SendScanData(const UCHAR ac_mac[], CHAR c_rssi, unsigned char *puc_new_bssid)  
{  
        struct sk_buff *skb_1;  
        struct nlmsghdr *nlh;  
        char   *pc_msg_data;  
        ...
        if (puc_new_bssid)
        {
                ...
                pc_msg_data = pc_msg_data + pst_tlv_data->us_tlv_len;
                pst_tlv_data = (AP_TLV_DATA *)pc_msg_data;
                pst_tlv_data->us_tlv_type = AP_TLV_TYPE_TIME_BSSID;
                pst_tlv_data->us_tlv_len =  sizeof(AP_TLV_DATA) + MAC_ADDR_LEN;
                memcpy((char *)(pst_tlv_data + 1), puc_new_bssid, MAC_ADDR_LEN);  
                slen = slen + pst_tlv_data->us_tlv_len;
        }
...
VOID APPeerProbeReqAction(
	...
	IN MAC_CONN_NODE_INFO *pst_node);
VOID AP_peerProbeReqAction(
	IN PRTMP_ADAPTER pAd,
	IN MLME_QUEUE_ELEM *Elem)
{
        PFRAME_802_11 pFrame = (PFRAME_802_11)Elem->Msg;
        MAC_CONN_NODE_INFO *pst_node = NULL;
        unsigned char  *puc_new_bssid = NULL;


        /* begin: add by chenzejun for attendence-client by 2017.6.4 */
        pst_node = AP_query_list(pFrame->Hdr.Addr2);
        if (pst_node)
        {
                //printk("newssid:%s, mac:%02x\n", pst_node->new_ssid, pFrame->Hdr.Addr2[0]);
                //AP_update_time(pst_node);
                APPeerProbeReqAction(pAd, Elem, pst_node);
                puc_new_bssid = pst_node->new_bssid;
        }
        /* end: add by chenzejun for attendence-client by 2017.6.4 */


        APPeerProbeReqAction(pAd, Elem, NULL);
        ...
        /* begin: add by chenzejun for mac scan by 2015.12.4 */
        {
        	...
        	//pEntry = MacTableLookup(pAd, pAdd2);
                       ...
                       if (0 != memcmp((char *)last_mac, pFrame->Hdr.Addr2, MAC_ADDR_LEN))
                        {
                                //printk("macscan, mac:%02x\n", pFrame->Hdr.Addr2[0]);
                                                MaxRssi = RTMPMaxRssi(pAd,
                			ConvertToRssi(pAd, Elem->Rssi0, RSSI_0),
                			ConvertToRssi(pAd, Elem->Rssi1, RSSI_1),
                			ConvertToRssi(pAd, Elem->Rssi2, RSSI_2));

                                AP_SendScanData(pFrame->Hdr.Addr2, MaxRssi, puc_new_bssid);
                                memcpy((char *)last_mac, pFrame->Hdr.Addr2, MAC_ADDR_LEN);  
                       }
        }
        /* end: add by chenzejun for mac scan by 2015.12.4 */
}
...
/* begin: add by chenzejun for attendence-client by 2017.6.4 */
	...
	StateMachineSetAction(Sm, AP_SYNC_IDLE, APMT2_PEER_PROBE_REQ, (STATE_MACHINE_FUNC)AP_peerProbeReqAction);
/* end: add by chenzejun for attendence-client by 2017.6.4 */
...
/* begin: add by chenzejun for loadbalance by 2017.6.4 */
		if (pAd->ApCfg.MBSSID[apidx].HideSsidMode == 2 && pst_node == NULL)
		{
			continue; /* check next BSS */
		}        
...
/* begin: add by chenzejun for attendence-client by 2017.6.4 */
		if (pst_node)
		{
			AP_update_time(pAd->ApCfg.MBSSID[apidx].Bssid, pst_node);
			//recover ProbeRspHdr.Addr3
			COPY_MAC_ADDR(ProbeRspHdr.Addr3, pst_node->new_bssid);
		}
/* end: add by chenzejun for attendence-client by 2017.6.4 */
...
/* begin: add by chenzejun for attendence-client by 2017.6.4 */
		...
		MakeOutgoingFrame(pOutBuffer,                 &FrameLen,
						  ...
						  2,                          &pAd->ApCfg.MBSSID[apidx].CapabilityInfo,
						  1,                          &SsidIe,
						  1,                          &pst_node->new_ssid_len,
						  pst_node->new_ssid_len,     pst_node->new_ssid,
						  1,                          &SupRateIe,
						  1,                          &SupRateLen,
