...
static void print_usage() {
    ...
    fprintf(stdout, "Usage: exttool --chanswitch"
     ...
     fprintf(stdout, "Usage: exttool --scan"
            " --interface wifiX"
            " --mindwell <min dwell time>"
            " --maxdwell <max dwell time>"
            " --resttime <rest time >"
            " --scanmode <1: ACTIVE, 0: PASSIVE> "
            " --chcount < Channel count and chan number >\n");
}
...
int validate_scan_param( wifi_scan_custom_request_cmd_t *scan_req)
{
    if ((scan_req->min_dwell_time <= 0) ||
        (scan_req->min_dwell_time > MIN_DWELL_TIME)) {
        fprintf(stdout, "Invalid min dwell time \n");
        return INVALID_ARG;
    }

    if ((scan_req->max_dwell_time <= 0) ||
        (scan_req->max_dwell_time > MAX_DWELL_TIME)) {
