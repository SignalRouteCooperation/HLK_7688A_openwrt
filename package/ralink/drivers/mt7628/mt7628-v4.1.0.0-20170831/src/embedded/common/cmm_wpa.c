...
/* begin: add by chenzejun for fault manage by 2017.8.4 */
extern void AP_SendFaultData(const UCHAR ac_mac[], unsigned char *puc_info) ;
/* end: add by chenzejun for fault manage by 2017.8.4 */
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
 ...
 /* begin: add by chenzejun for apcli verify by 2017.1.30 */
                                      ...
                                      ///
                                      if (pApCliTab->PMKNextId >= APCLI_PSK_COUNT)   pApCliTab->PMKNextId = 0;
                                      if (pApCliTab->PMKNextId !=  pApCliTab->PMKCurrentId)   pApCliTab->PMKCurrentId = pApCliTab->PMKNextId;

                                      if ((pApCliTab->bApCliVerifyEnable == TRUE) && (pApCliTab->PMKCurrentId < APCLI_PSK_COUNT)) 
                                      {
                                             /*
                                             printk("PeerPairMsg1Action begin, PMKCurrentId:%d, Password:%s, PMK:0x%02x%02x\n", 
                                                        pApCliTab->PMKCurrentId, 
                                                        pApCliTab->stApcliVerify[pApCliTab->PMKCurrentId].Password,
                                                        pApCliTab->stApcliVerify[pApCliTab->PMKCurrentId].PMK[0],
                                                        pApCliTab->stApcliVerify[pApCliTab->PMKCurrentId].PMK[1]);
                                             */
                                                                                          
                                             //change next pmk, try to connect
                                             pmk_ptr = pApCliTab->stApcliVerify[pApCliTab->PMKCurrentId].PMK;
                                      }
#endif
#endif
 /* end: add by chenzejun for apcli verify by 2017.1.30 */
...
#ifdef APCLI_SUPPORT
 #if 0
             pEntry->VerifyTimeOutCount++;

             //printk("<=== PeerPairMsg1Action: send Msg2 of 4-way, timeout:%d, %d\n", pEntry->VerifyTimeOutCount, pApCliTab->bApCliVerifyEnable);

             ///4 ??
             if (pApCliTab && pApCliTab->bApCliVerifyEnable && (pEntry->VerifyTimeOutCount >= 4))
             {
                        if (pApCliTab->PMKCurrentId < APCLI_PSK_COUNT)
                        {
                                //false
                                pApCliTab->stApcliVerify[pApCliTab->PMKCurrentId].bVerifyResult = FALSE;
                                pEntry->VerifyTimeOutCount = 0;
                                pApCliTab->PMKNextId = pApCliTab->PMKCurrentId + 1;
                                //printk("bVerifyResult:false, next PMKCurrentId:%d, \n", pApCliTab->PMKCurrentId);
                        }
             }
#endif
#endif
    ...
    IN MLME_QUEUE_ELEM  *Elem)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		pmk_ptr = pAd->ApCfg.MBSSID[apidx].PMK;
                         ...
                         #if 0 //#ifdef AUTOLINK_SSID_SUPPORT	
                          if (pEntry->mac_black_flag)
                          {
                                 //printk("PeerPairMsg2Action,    PMK:%02x%02x%02x%02x%02x%02x%02x%02x\n", 
                                 //                               pEntry->mac_PMK[0], pEntry->mac_PMK[1], pEntry->mac_PMK[2],pEntry->mac_PMK[3],
                                  //                              pEntry->mac_PMK[4], pEntry->mac_PMK[5], pEntry->mac_PMK[6],pEntry->mac_PMK[7]);
		        pmk_ptr = pEntry->mac_PMK;
                          }
                          #endif
...
/* begin: add by chenzejun for apcli verify by 2017.1.30 */
                                      ...
                                      //printk("<=== PeerPairMsg3Action, timeout:%d, %d\n", pEntry->VerifyTimeOutCount, pApCliTab->bApCliVerifyEnable);
                                      if (pApCliTab && pApCliTab->bApCliVerifyEnable && (pApCliTab->PMKCurrentId < APCLI_PSK_COUNT))
                                      {
                                                //TRUE
                                                pApCliTab->stApcliVerify[pApCliTab->PMKCurrentId].bVerifyResult = TRUE;
                                                pApCliTab->PMKNextId = pApCliTab->PMKCurrentId + 1;
                                                //printk("pEntry->bMainVerifyResult = true, id:%d\n", pApCliTab->PMKCurrentId);
                                                return;
                                       }                                      
#endif
#endif
...
{
        CHAR message[100] = {0};
        snprintf(message, 100, "Send DEAUTH, size:%d, Reason:%d", pAd->MacTab.Size, Reason);
        AP_SendFaultData(pEntry->Addr, message);
}
/* end: add by chenzejun for fault manage by 2017.8.4 */
