...
extern void ol_rx_mesh_mode_per_pkt_rx_info(adf_nbuf_t nbuf, struct ol_txrx_peer_t *peer, struct ol_txrx_vdev_t *vdev);
#endif
    ...
    adf_nbuf_t nbuf_list)
{
    ...
    while (nbuf) {
        ...
        adf_nbuf_t next = adf_nbuf_next(nbuf);
        ...
        if (discard) {
             ...
             */
            byte_cnt += (adf_nbuf_len(nbuf) + tot_frag_cnt);
#endif
        }
    ...
    adf_nbuf_t *next_mpdu)
{
    ...
    if (adf_nbuf_is_chfrag_start(nbuf)) {
        ...
        TXRX_ASSERT2(nbuf);
        while (!adf_nbuf_is_chfrag_end(nbuf))
        {
            nbuf = adf_nbuf_next(nbuf);
            TXRX_ASSERT2(nbuf);
        }
