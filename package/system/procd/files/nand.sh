...
nand_kt_copy_config() {
	...
	echo "Copy Nand config..."
	cp -af "$CONF_TAR" $OPT_PATH/sysupgrade.tgz
}
...
nand_kt_restore_config() {
	...
	CONF_TAR="$OPT_PATH/sysupgrade.tgz"
	[ -f $CONF_TAR ] && {
		echo "- config restore -"
		cd /
		tar xzf $CONF_TAR
...
nand_kt_mount_opt() {
	...
	local ubidevdir ubivoldir ubidevname
	ubidevdir="/sys/devices/virtual/ubi/$ubidev"
	[ ! -d "$ubidevdir" ] && return 1
	for ubivoldir in $ubidevdir/${ubidev}_*; do
		[ ! -d "$ubivoldir" ] && continue
		if [ "$( cat $ubivoldir/name )" = "$CI_UBIPART" ]; then
			ubidevname=$(basename $ubivoldir) 
		fi
	done
	
	mkdir $OPT_PATH
	if ! mount -t ubifs $ubidevname $OPT_PATH; then
		echo "mounting ubifs $CI_UBIPART failed"	
	fi	
}
#end zhangzf
