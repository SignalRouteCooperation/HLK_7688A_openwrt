...
#define  	MCL_HASHSIZE	32

struct ieee80211_mcl_entry
{
	TAILQ_ENTRY(ieee80211_mcl_entry)     me_list;
	LIST_ENTRY(ieee80211_mcl_entry)      me_hash; 
	u_int8_t                             me_macaddr[IEEE80211_ADDR_LEN];
};

struct ieee80211_mcl
{
    osdev_t                              mcl_osdev;
    spinlock_t                           mcl_lock;
    int	                                 mcl_sum;
    TAILQ_HEAD(, ieee80211_mcl_entry)    mcl_list; /* list of all mcl_entries */
    ATH_LIST_HEAD(, ieee80211_mcl_entry) mcl_hash[MCL_HASHSIZE];
};

typedef struct ieee80211_mcl  *ieee80211_mcl_t;
