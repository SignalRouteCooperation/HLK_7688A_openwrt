...
static int populate_report_static(ath_spectralsim_report *report,
                              ...
                              bool is_80_80)
{
    adf_os_assert_always(report != NULL);
    ...
    switch(width)
    {
        case IEEE80211_CWM_WIDTH20:
            ...
            report->data = (u_int8_t*)
                adf_os_mem_alloc(NULL, sizeof(reportdata_20));
            ...
            adf_os_mem_copy(report->data,
                    reportdata_20, report->datasize);
   
            adf_os_mem_copy(&report->rfqual_info,
                    &rfqual_info_20, sizeof(report->rfqual_info));
            
            adf_os_mem_copy(&report->chan_info,
        ...
        case IEEE80211_CWM_WIDTH40:
            report->data = NULL;
            report->data = (u_int8_t*)
                adf_os_mem_alloc(NULL, sizeof(reportdata_40));
        ...
        case IEEE80211_CWM_WIDTH80:
            report->data = NULL;
            report->data = (u_int8_t*)
                adf_os_mem_alloc(NULL, sizeof(reportdata_80));
        ...
        case IEEE80211_CWM_WIDTH160:
            if (is_80_80) {
                report->data = NULL;
                report->data = (u_int8_t*)
                    adf_os_mem_alloc(NULL, sizeof(reportdata_80_80));
                ...
                report->data = NULL;
                report->data = (u_int8_t*)
                    adf_os_mem_alloc(NULL, sizeof(reportdata_160));
                              ...
                              bool is_80_80)
{
    ...
    adf_os_assert_always(reportset != NULL);
    ...
    report = (ath_spectralsim_report*)
        adf_os_mem_alloc(NULL, sizeof(ath_spectralsim_report));
    ...
    adf_os_mem_zero(report, sizeof(*report));

    switch(width)
    {
        case IEEE80211_CWM_WIDTH20:
            adf_os_mem_copy(&reportset->config,
...
OS_TIMER_FUNC(spectral_sim_phyerrdelivery_handler)
{
    ...
    adf_os_assert_always(spectral != NULL);

    simctx = (ath_spectralsim_context *)spectral->simctx;
    adf_os_assert_always(simctx != NULL);
    ...
    adf_os_assert_always(curr_reportset != NULL);

    curr_report = curr_reportset->curr_report;
    adf_os_assert_always(curr_report != NULL);

    adf_os_assert_always(curr_reportset->headreport != NULL);
...
int spectral_sim_attach(struct ath_spectral *spectral)
{
    ...
    simctx = (ath_spectralsim_context*)
                adf_os_mem_alloc(NULL, sizeof(ath_spectralsim_context)); 
    ...
    adf_os_mem_zero(simctx, sizeof(*simctx));

    spectral->simctx = simctx;

    if (populate_simdata(simctx) != 0) {
        adf_os_mem_free(simctx);
...
u_int32_t spectral_sim_configure_params(void* arg, HAL_SPECTRAL_PARAM* params)
{
    ...
    adf_os_assert_always(params != NULL); 
    ...
    while(temp_reportset) {
        if (adf_os_mem_cmp(&temp_reportset->config,
...
u_int32_t spectral_sim_get_params(void* arg, HAL_SPECTRAL_PARAM* params)
{
    ...
    adf_os_mem_copy(params, &simctx->curr_reportset->config, sizeof(*params));
