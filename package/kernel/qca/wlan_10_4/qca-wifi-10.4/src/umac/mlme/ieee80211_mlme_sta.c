...
int wlan_mlme_join_infra(wlan_if_t vaphandle, wlan_scan_entry_t bss_entry, u_int32_t timeout)
{
    ...
    if (error == EOK) {
        ...
        printk("%s: ni->ni_chan->ic_ieee:%d ni->ni_chan flags:%x ni->ni_chan:%p\n", __func__, ni->ni_chan->ic_ieee, ni->ni_chan->ic_flags, ni->ni_chan);
#if ATH_SUPPORT_WRAP
        if (!ic->ic_mpsta_vap || (ic->ic_mpsta_vap && vap->iv_mpsta)) {
#endif
            IEEE80211_DPRINTF(vap, IEEE80211_MSG_MLME, "Setting channel number %d\n", phychan->ic_ieee);
            printf("%s.. SETTING CHANNEL NUMBER %d CURR CHANNEl is %d flags 0x%x phychan:%p\n",
                __func__, phychan->ic_ieee, vap->iv_ic->ic_curchan->ic_ieee, phychan->ic_flags, phychan);
