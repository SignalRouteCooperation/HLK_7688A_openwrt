...
static inline
struct rx_desc_base *
ar_rx_desc(adf_nbuf_t msdu)
{
#if (QCA_PARTNER_DIRECTLINK_RX || QCA_PARTNER_CBM_DIRECTPATH)
    if(is_partner_buffer(msdu))
    {
        return (struct rx_desc_base *)
            ((((size_t) (adf_nbuf_head(msdu) + NBUF_HEADER_RESERVED_PARTNER + RX_DESC_ALIGN_MASK)) &
            ~RX_DESC_ALIGN_MASK) + RX_DESC_ALIGN_RESERVED);
    }
#endif
    return
        (struct rx_desc_base *)
        ((((size_t) (adf_nbuf_head(msdu) + RX_DESC_ALIGN_MASK)) &
          ~RX_DESC_ALIGN_MASK) + RX_DESC_ALIGN_RESERVED);
}
...
static inline 
adf_nbuf_t 
rx_adf_noclone_buf(adf_nbuf_t buf)
{
    return buf;
}
