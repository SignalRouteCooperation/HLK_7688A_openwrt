...
VOID APCheckBcnQHandler(RTMP_ADAPTER *pAd, INT apidx, BOOLEAN *is_pretbtt_int)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("flush result = %x\n", temp));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("check pse fid Q7:"));
	set_get_fid(pAd, "7");
...
VOID PSEWatchDog(RTMP_ADAPTER *pAd)
{
	...
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PSE Reset:MonitorRxPse\n"));
		pAd->PSEResetCount++;
		goto reset;
	}
	...
	if ((NoDataIn) 
		&& (pAd->pse_reset_exclude_flag == FALSE))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PSE Reset:MonitorTxBcnRxPse\n"));
		pAd->PSEResetCount++;
		goto reset;
	}
#endif /* RTMP_PCI_SUPPORT */
