...
static int
wmi_thermal_stats_event_handler(ol_scn_t scn, u_int8_t *data, u_int16_t datalen, void *context)
{
    ...
    tt_stats_event = (tt_stats_t *) data;
    scn->thermal_param.th_stats.temp = tt_stats_event->temp;
    scn->thermal_param.th_stats.level = tt_stats_event->level;
    for (i = 0; i < THERMAL_LEVELS; i++) {
        scn->thermal_param.th_stats.levstats[i].levelcount = tt_stats_event->levstats[i].levelcount;
        scn->thermal_param.th_stats.levstats[i].dccount = tt_stats_event->levstats[i].dccount;
    }
...
int32_t ol_ath_config_thermal_mitigation_param(struct ol_ath_softc_net80211 *scn)
{
    ...
    if (!buf) {
        TH_DEBUG_PRINT(TH_DEBUG_LVL0, scn,"%s:wmi_buf_alloc failed\n", __FUNCTION__);
        return -ENOMEM;
    }

    cmd = (tt_config_t*) wmi_buf_data(buf);
    cmd->enable = scn->thermal_param.th_cfg.enable;
    cmd->dc = scn->thermal_param.th_cfg.dc;
    cmd->dc_per_event = scn->thermal_param.th_cfg.dc_per_event;
    for (i = 0; i < THERMAL_LEVELS; i++) {
        cmd->levelconf[i].tmplwm = scn->thermal_param.th_cfg.levelconf[i].tmplwm;
        cmd->levelconf[i].tmphwm = scn->thermal_param.th_cfg.levelconf[i].tmphwm;
        cmd->levelconf[i].dcoffpercent = scn->thermal_param.th_cfg.levelconf[i].dcoffpercent;
        cmd->levelconf[i].prio = scn->thermal_param.th_cfg.levelconf[i].priority;
    }
    ...
    error = wmi_unified_cmd_send(scn->wmi_handle, buf, len, WMI_TT_SET_CONF_CMDID);
    if (error) {
        TH_DEBUG_PRINT(TH_DEBUG_LVL0, scn," %s :WMI Failed \n", __func__);
    }
...
int32_t __ol_ath_thermal_mitigation_attach(struct ol_ath_softc_net80211 *scn)
{
    ...
    if (get_tt_supported(scn)) {
        ...
        retval = wmi_unified_register_event_handler(scn->wmi_handle, WMI_TT_STATS_EVENTID,
                wmi_thermal_stats_event_handler, NULL);
        if (!retval) {
            retval = ol_ath_config_thermal_mitigation_param(scn);
            if (retval) {
                wmi_unified_unregister_event_handler(scn->wmi_handle, WMI_TT_STATS_EVENTID);
            }
        }
    }
...
int32_t __ol_ath_thermal_mitigation_detach(struct ol_ath_softc_net80211 *scn)
{
    ...
    if (get_tt_supported(scn)) {
        ...
        retval = ol_ath_config_thermal_mitigation_param(scn);
        retval = wmi_unified_unregister_event_handler(scn->wmi_handle, WMI_TT_STATS_EVENTID);
    }
