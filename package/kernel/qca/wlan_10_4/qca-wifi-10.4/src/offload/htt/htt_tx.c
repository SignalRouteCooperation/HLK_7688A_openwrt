...
htt_peer_map_timer_init(struct ol_txrx_pdev_t *pdev)
{
    ...
    adf_os_print("%s Enter pdev %p hrtimer %p\n",__FUNCTION__, pdev, &pdev->qmap_ctxt);
...
htt_alloc_peer_map_mem(struct htt_pdev_t *pdev, int desc_pool_elems)
{
     ...
     */
    adf_os_mem_set((uint8_t *)&pdev->host_q_mem, 0, sizeof(pdev->host_q_mem));
    ...
    pdev->host_q_mem.pool_vaddr = adf_os_mem_alloc(pdev->osdev, pool_size);
    ...
    pdev->host_q_mem.pool_paddr = paddr_lo = (uint32_t )OS_DMA_MAP_SINGLE(pdev->osdev, vaddr,
                                                                   pdev->host_q_mem.size,ADF_OS_DMA_TO_DEVICE);
...
htt_tx_attach(struct htt_pdev_t *pdev, int desc_pool_elems)
{
    ...
    intr_ctxt = (in_interrupt() || irqs_disabled()) ? 1 : 0;
        pdev->tx_descs.pool_vaddr = (char*)wifi_cmem_allocation( ic->interface_id,
                CM_TX_ATTACH_POOL_VADDR,pool_size,(void*) scn->adf_dev->drv_hdl,&pool_paddr,intr_ctxt);
 #else
	...
	pool_size = pdev->frag_descs.pool_elems * pdev->frag_descs.size;
        ...
        pdev->frag_descs.pool_vaddr = (char*)wifi_cmem_allocation( ic->interface_id,
           CM_TX_ATTACH_FRAG_POOL_VADDR,pool_size,(void*) scn->adf_dev->drv_hdl,&pool_paddr,intr_ctxt);
 #else

	pdev->frag_descs.pool_vaddr = adf_os_mem_alloc_consistent(
			pdev->osdev, pool_size, &pool_paddr,
			adf_os_get_dma_mem_context((&pdev->frag_descs), memctx));
#endif
	...
	*p = NULL;
	...
	adf_os_spinlock_init(&pdev->htt_buf_list_lock);
...
htt_tx_detach(struct htt_pdev_t *pdev)
{
 ...
 wifi_cmem_free(ic->interface_id, CM_TX_ATTACH_FRAG_POOL_VADDR , (int)pdev->frag_descs.pool_vaddr);
    ...
    adf_os_mem_free(pdev->host_q_mem.pool_vaddr);
#endif
}
...
htt_tx_mgmt_desc_alloc(struct htt_pdev_t *pdev, A_UINT8 vdev_id,
                       A_UINT32 *desc_id, adf_nbuf_t mgmt_frm)
{
    adf_nbuf_t msg_buf;
    ...
    if (pdev->tx_mgmt_desc_ctxt.freelist) {
        ...
        *desc_id = pdev->tx_mgmt_desc_ctxt.freelist->desc_id;
        ...
        adf_os_mem_zero(adf_nbuf_data(msg_buf), HTT_MSG_BUF_SIZE(sizeof(struct htt_mgmt_tx_desc_t)));
        return msg_buf;
    }
...
htt_tx_mgmt_desc_free(struct htt_pdev_t *pdev, A_UINT8 desc_id, A_UINT32 status)
{
    ...
    if(!mgmt_frm){
        adf_os_spin_unlock_bh(&pdev->htt_tx_mutex);
        adf_os_print("ERROR: %s: unexpected mgmt frame: NULL for desc_id: %d\n", __func__, desc_id);
        return;
    }
    ...
    adf_nbuf_unmap_single(pdev->txrx_pdev->osdev, mgmt_frm, ADF_OS_DMA_TO_DEVICE);
                                   ...
                                   A_UINT32 status)
{
    ...
    for(desc_id = 0; desc_id < HTT_MAX_NUM_MGMT_DESCS; desc_id++) {
         ...
         * from completion handler will finish first and mgmt_frm (skb) will be freed.
...
#define TX_MGMT_DONE_WAIT_TIMEOUT 100
    ...
    for(i = 0; i < HTT_MAX_NUM_MGMT_DESCS ; i++) {
        adf_nbuf_free(msg_pool[i].msg_buf);
    }
...
htt_tx_fetch_desc_pool_alloc(struct htt_pdev_t *pdev, A_UINT32 num_elems)
{
    ...
    msg_pool = (struct htt_tx_fetch_desc_buf *)adf_os_mem_alloc(pdev->osdev,
                                    (num_elems *
...
#define TX_FETCH_DONE_WAIT_TIMEOUT 100
    ...
    adf_os_mem_free(pdev->tx_fetch_desc_ctxt.pool);
...
htt_tx_fetch_desc_free(struct htt_pdev_t *pdev, A_UINT32 desc_id)
{
    ...
    adf_os_assert(desc_id < HTT_MAX_NUM_FETCH_DESCS);
    msg_pool = pdev->tx_fetch_desc_ctxt.pool ;
    /* acquire lock here */
    adf_os_spin_lock_bh(&pdev->htt_tx_mutex);
    ...
    adf_nbuf_unmap_single( pdev->osdev, pdev->tx_fetch_desc_ctxt.pool[desc_id].msg_buf , ADF_OS_DMA_TO_DEVICE);
	adf_nbuf_free(msg_pool[desc_id].msg_buf);
        ...
        enum htt_pkt_type pkt_type)
{
    ...
    adf_os_mem_zero(buff,htc_htt_hdr_size);
        ...
        int *hdrcache)
{
	...
	adf_os_mem_copy(hdrcache, pdev->htt_hdr_cache, htc_htt_hdr_size);
}
