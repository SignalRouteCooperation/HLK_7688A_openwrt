...
#include <ol_txrx_ctrl_api.h>  /* ol_txrx_get_vdev_rx_decap_type, etc. */
...
extern adf_nbuf_t adf_nbuf_alloc_partner(struct htt_pdev_t *pdev, adf_os_size_t size, int reserve, int align, int prio);
#endif
...
u_int32_t htt_rx_ind_fw_desc_bytes_get(rx_handle_t rxh, adf_nbuf_t rx_ind_msg)
{
    u_int32_t *msg_word;
...
htt_rx_ring_fill_n(struct htt_pdev_t *pdev, int num)
{
    ...
    while (num > 0) {
        ...
        int headroom;
#if QCA_PARTNER_CBM_DIRECTPATH
        rx_netbuf = adf_nbuf_alloc_partner(pdev, HTT_RX_BUF_SIZE,0, 4, FALSE);
#else
...
static inline
void
htt_set_checksum_result_hl( adf_nbuf_t msdu, void *rx_desc)
{
    ...
    switch ((is_udp << 2) | (is_tcp << 1) | (is_ipv6 << 0)) {
        ...
        case 0x3:
            ...
            break;
        default:
            cksum.l4_type = ADF_NBUF_RX_CKSUM_TYPE_NONE;
            break;
    }
...
void ol_ath_arp_debug(rx_handle_t rxh, adf_nbuf_t msdu);
...
ar_rx_attach(ar_handle_t arh, rx_handle_t htt_pdev, adf_os_device_t osdev, struct htt_rx_callbacks *cbs);
...
htt_rx_attach(struct htt_pdev_t *pdev)
{
    ...
    if (!pdev->cfg.is_high_latency) {
        ...
        pdev->rx_ring.buf.paddrs_ring = wifi_cmem_allocation( ic->interface_id,CM_RX_ATTACH_PADDR_RING,pdev->rx_ring.size * sizeof(u_int32_t),(void*) scn->adf_dev->drv_hdl,&paddr,intr_ctxt);
#else
        ...
        pdev->rx_ring.alloc_idx.vaddr =wifi_cmem_allocation( ic->interface_id,
                CM_RX_ATTACH_ALLOC_IDX_VADDR,
                                           sizeof(u_int32_t),(void*) scn->adf_dev->drv_hdl,&paddr,intr_ctxt);
#else
