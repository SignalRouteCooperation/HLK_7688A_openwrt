...
void AP_Netlink_init(void)
{
        ...
        if (p_nl_sock == NULL)
        {
                p_nl_sock = netlink_kernel_create(&init_net, NETLINK_5G, &st_netlink_cfg);
                if(!p_nl_sock)  
                {  
...
/* begin: add by chenzejun for loadbalance by 2017.6.4 */
		if (pAd->ApCfg.MBSSID[apidx].HideSsidMode == 2)
		{
			continue; /* check next BSS */
		}        
	...
	BND_STRG_CHECK_CONNECTION_REQ(	pAd,
...
#if 0
                	{
                		...
                		//pEntry = MacTableLookup(pAd, pAdd2);
                                       ...
                                       if (0 != memcmp((char *)last_mac, Addr2, MAC_ADDR_LEN))
                                        {
                			...
                			ConvertToRssi(pAd, Elem->Rssi2, RSSI_2));
                                                 ...
                                                 AP_SendData(Addr2, MaxRssi);
                                                memcpy((char *)last_mac, Addr2, MAC_ADDR_LEN);  
                                       }
                	}
#endif
                          /* end: add by chenzejun for mac scan by 2015.12.4 */
