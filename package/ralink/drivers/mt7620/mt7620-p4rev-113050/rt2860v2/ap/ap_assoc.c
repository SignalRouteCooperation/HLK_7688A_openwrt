...
/* begin: add by chenzejun for loadbalance by 2017.6.4 */
	if (pAd->ApCfg.MBSSID[pEntry->apidx].HideSsidMode == 2)
	{
		goto LabelOK;
	}        
...
/*	UCHAR 		KeyIdx; */
    ...
    if (pEntry && ((pEntry->Sst == SST_AUTH) || (pEntry->Sst == SST_ASSOC)))
    {
        ...
        *pAid = pEntry->Aid;
        ...
        /* begin: add by chenzejun for mac scan by 2016.1.22 */
                AP_SendConnectData(pEntry->Addr, pAd->RFICType, pAd->CommonCfg.TxPowerPercentage, pAd->MacTab.Size);
        /* end: add by chenzejun for mac scan by 2016.1.22 */
        
                                        /* begin: add by chenzejun for mac filter by 2016.1.30 */
                                        #if 0 //#ifdef AUTOLINK_SSID_SUPPORT	
                                        if ((pEntry->AuthMode >= Ndis802_11AuthModeWPA) && (pAd->ApCfg.MBSSID[pEntry->apidx].bAutoLinkEnable == TRUE))
                                        {
                                                int i = 0;
                                                    ...
                                                    for (i = 0; i < 64; i++)
                                                   {
                                                    ...
                                                    if (g_cfgdev_debug_flag == 1) 
                                                    {
                                                                printk("ASSOCATE, line:%d, mac_white_flag:%d, apidx:%d, ssid:%s, MAC:%02x%02x%02x%02x%02x%02x\n", 
                                                                        __LINE__,
                                                                        pEntry->mac_white_flag, pEntry->apidx, pAd->ApCfg.MBSSID[pEntry->apidx].Ssid, 
