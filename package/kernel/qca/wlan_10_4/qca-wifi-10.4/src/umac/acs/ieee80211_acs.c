...
static int ieee80211_get_chan_ssid_list(void *arg, wlan_scan_entry_t se);
    ...
    switch (channel->ic_flags & mode_mask)
    {
        ...
        case IEEE80211_CHAN_11AC_VHT160:
            ...
            if(pri_center_ch_diff > 0) {
                ...
                sec_chan_80_1 = channel->ic_vhtop_ch_freq_seg1 - SEC_80_4;
                sec_chan_80_2 = channel->ic_vhtop_ch_freq_seg1 - SEC_80_3;
                sec_chan_80_3 = channel->ic_vhtop_ch_freq_seg1 - SEC_80_2;
                sec_chan_80_4 = channel->ic_vhtop_ch_freq_seg1 - SEC_80_1;
            }
            else {
                sec_level = UPPER_FREQ_SLOT;
                sec_chan_80_1 = channel->ic_vhtop_ch_freq_seg1 + SEC_80_1;
                sec_chan_80_2 = channel->ic_vhtop_ch_freq_seg1 + SEC_80_2;
                sec_chan_80_3 = channel->ic_vhtop_ch_freq_seg1 + SEC_80_3;
                sec_chan_80_4 = channel->ic_vhtop_ch_freq_seg1 + SEC_80_4;
            }
            /* Derive Secondary 20, 40 from Primary 80 */
            pri_center_ch = channel->ic_vhtop_ch_freq_seg1 - PRI_80_CENTER*sec_level;
            ...
            /* Adjacent channels are 4 channels before the band and 4 channels are
               after the band */
            first_adj_chan = (channel->ic_vhtop_ch_freq_seg1 - 14) - 2*ADJ_CHANS;
            last_adj_chan =  (channel->ic_vhtop_ch_freq_seg1 + 14) + 2*ADJ_CHANS;
...
static int
ieee80211_get_chan_ssid_list(void *arg, wlan_scan_entry_t se)
{
    ieee80211_chan_ssid_list *chan_ssid_list = (ieee80211_chan_ssid_list *) arg ;
    ...
    ssid = (char *)ieee80211_scan_entry_ssid(se, &len);
    /* sanity check */
    if ((chan_ssid_list== NULL) || (chan_ssid_list->acs == NULL) || (chan_ssid_list->acs->acs_ic == NULL) || (ssid == NULL)) {
        eacs_trace(EACS_DBG_OBSS,("vap/acs/ic/ssid is null\n"));
        return EINVAL;
    }
    acs = chan_ssid_list->acs;
    ic = acs->acs_ic;
    nbss = chan_ssid_list->nbss;
    ...
    if ((channel_ieee_se == chan_ssid_list->chan) && (nbss < IEEE80211_MAX_SSID)) {
        memcpy(chan_ssid_list->ssid_list[nbss].ssid, ssid, strlen(ssid));
        chan_ssid_list->nbss++;
    }
...
int ieee80211_acs_scan_report(struct ieee80211com *ic, struct ieee80211_acs_dbg *acs_r, u_int8_t internal)
{
    ...
    u_int16_t len;
    u_int8_t *ssid;
    ieee80211_chan_ssid_list *acs_ssid_list = NULL;
    ...
    acs_ssid_list = (ieee80211_chan_ssid_list *) OS_MALLOC(acs->acs_osdev,
            sizeof(ieee80211_chan_ssid_list), 0);

    if (acs_ssid_list) {
        OS_MEMZERO(acs_ssid_list, sizeof(ieee80211_chan_ssid_list));
    }
    else {
        adf_os_print("%s: malloc failed \n",__func__);
        return 0;
    ...
    acs_ssid_list->acs = acs;
    acs_ssid_list->chan = ieee_chan;

    wlan_scan_table_iterate(acs->acs_vap, ieee80211_get_chan_ssid_list, (void *)acs_ssid_list);

    /* coping the list of SSID in the channel */
    for (count = 0; count < acs->acs_chan_nbss[ieee_chan]; count++) {
        ssid = acs_ssid_list->ssid_list[count].ssid;
        len = strlen(ssid);
        memcpy(acs_r->ssid_list[count].ssid, ssid, len);
        acs_r->ssid_list[count].len = len;
    }
...
static void ieee80211_acs_scan_report_internal(struct ieee80211com *ic)
{
    ...
    acs_dbg = (struct ieee80211_acs_dbg *) OS_MALLOC(acs->acs_osdev,
            IEEE80211_CHAN_MAX * sizeof(struct ieee80211_acs_dbg), 0);

    if (acs_dbg) {
        OS_MEMZERO(acs_dbg, sizeof(struct ieee80211_acs_dbg)*IEEE80211_CHAN_MAX);
    }
    else {
        adf_os_print("%s: malloc failed \n",__func__);
        return;
