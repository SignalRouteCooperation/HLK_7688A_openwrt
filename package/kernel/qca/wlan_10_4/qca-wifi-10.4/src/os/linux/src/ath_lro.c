...
#define IPV4_TCP_PAYLOAD_LENGTH(ip4_header, tcp_header) \
        (adf_os_ntohs(ip4_header->tot_len) - IP_HEADER_LEN(ip4_header) - \
	TCP_HEADER_LEN(tcp_header))
#define IPV6_TCP_PAYLOAD_LENGTH(ip6_header, tcp_header) \
        (adf_os_ntohs(ip6_header->payload_len) - TCP_HEADER_LEN(tcp_header))
...
static inline void ath_get_nbuf_hdr(adf_nbuf_t nbuf,
                                    void **ip_header,
                                    void **tcp_header,
                                    ath_lro_hw_info_t *lro_info)
{
     ...
     */
    unsigned char *l3_start = adf_nbuf_data(nbuf);

    if (nbuf->protocol == adf_os_htons(ETH_P_8021Q)) {
        l3_start += VLAN_HLEN;
    }
                                     ...
                                     ath_lro_hw_info_t *lro_info)
{
    ...
    if (tcp_header->doff == TCP_TIMESTAMP_LEN) {
        ...
        if (*ts_ptr != adf_os_htonl((TCPOPT_NOP << 24) |
                                    (TCPOPT_NOP << 16) |
                                    (TCPOPT_TIMESTAMP << 8) |
                                    TCPOLEN_TIMESTAMP)) {
        ...
        if (le) {
            if (adf_os_ntohl(le->tcp_tsval) > adf_os_ntohl(*(ts_ptr + 1))) {
                return (ATH_TCP_LRO_FAILURE);
            }
        }
...
static void ath_lro_update_head_nbuf_header(ath_lro_entry_t *le)
{
    ...
    if (le->ipv6_proto) {
        ...
        ip6_header->payload_len = adf_os_htons(le->ip_total_len);
    }
    else {
        ip_header = (struct iphdr *)le->ip_header;
        csum_replace2(&ip_header->check,
                      ip_header->tot_len,
                      adf_os_htons(le->ip_total_len));

        ip_header->tot_len = adf_os_htons(le->ip_total_len);
    }

    tcp_header->check = 0;

    tcp_header_ck_sum = csum_partial(tcp_header, TCP_HEADER_LEN(tcp_header), 0);
    tcp_payload_ck_sum = ~csum_unfold(adf_os_htons(le->tcp_payload_ck_sum));
...
static void ath_lro_init_entry(ath_lro_entry_t *le, adf_nbuf_t nbuf,
                               ...
                               ath_lro_hw_info_t *lro_info)
{
    ...
    if (lro_info->ipv6_proto) {
        ...
        le->ip_total_len = adf_os_ntohs(ip6_header->payload_len);
        le->ipv6_proto = 1;
    }
    else {
        ip4_header = (struct iphdr *)ip_header;
        tcp_data_len = IPV4_TCP_PAYLOAD_LENGTH(ip4_header, tcp_header);
        le->ip_total_len = adf_os_ntohs(ip4_header->tot_len);
    }
    ...
    le->tcp_next_seq = adf_os_ntohl(tcp_header->seq) + tcp_data_len;
...
static void ath_lro_add_packet(ath_lro_entry_t *le, adf_nbuf_t nbuf,
                               void *ip_header, struct tcphdr *tcp_header,
                               ath_lro_hw_info_t *lro_info)
{
    ...
    adf_nbuf_pull_head(nbuf, (nbuf->len - tcp_data_len));
...
#else
        __vlan_hwaccel_put_tag(le->head, adf_os_htons(ETH_P_8021Q), osdev->vlanID);
#endif
    }
#else
...
unsigned int ath_lro_process_nbuf(adf_nbuf_t nbuf, void *priv)
{
    ath_lro_entry_t *le;
    void *ip_header;
    struct tcphdr *tcp_header;
    ...
    lro_info = NULL;
    lro_info = (ath_lro_hw_info_t *)adf_nbuf_get_cb(nbuf);
    ...
    if (le->tcp_next_seq != adf_os_ntohl(tcp_header->seq)) {
        ath_lro_flush(le);
        return (ATH_TCP_LRO_FAILURE);
    }
