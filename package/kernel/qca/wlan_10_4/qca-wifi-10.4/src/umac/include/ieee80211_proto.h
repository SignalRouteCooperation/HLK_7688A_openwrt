...
/*int ieee80211_send_proberesp(struct ieee80211_node *ni, u_int8_t *macaddr,
                            const void            *optie, 
                            const size_t          optielen,
                            struct ieee80211_framing_extractx *extractx, unsigned long unixtime);*/
 ...
 * @param [in] root_ap_dist  the distance from the root AP (in hops) for this
 *                           radio; a value of 0 indicates this device is the
 *                           root AP
 *
 * @return pointer to where the remainder of the frame should be written
 */
u_int8_t *ieee80211_add_whc_ap_info_ie(u_int8_t *frm, u_int16_t whc_caps, u_int8_t hop_count);
