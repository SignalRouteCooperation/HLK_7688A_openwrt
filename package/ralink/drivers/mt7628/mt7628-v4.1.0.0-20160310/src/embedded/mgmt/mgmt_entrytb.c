...
void AP_SendDeleteData(const UCHAR ac_mac[], USHORT ui_count)  
{  
        ...
        //DBGPRINT(RT_DEBUG_OFF, ("delete connect data and send msg\n"));
        //printk("delete connect data and send msg, mac:%02X%02X%02X%02X%02X%02X \n", ac_mac[0], ac_mac[1], ac_mac[2], ac_mac[3], ac_mac[4], ac_mac[5]);  
	...
	//tlv: count
        ...
        nlh->nlmsg_len = slen;
        nlh->nlmsg_type = AP_MSG_TYPE_CONN_DEL; 
...
BOOLEAN MacTableDeleteEntry(RTMP_ADAPTER *pAd, USHORT wcid, UCHAR *pAddr)
{
	...
	if (pEntry && !IS_ENTRY_NONE(pEntry))
	{
		...
		if (MAC_ADDR_EQUAL(pEntry->Addr, pAddr))
		{
			...
			pAd->MacTab.Size--;
			/* begin: add by chenzejun for mac scan by 2016.1.22 */
			AP_SendDeleteData(pEntry->Addr, pAd->MacTab.Size);
			NdisZeroMemory(pEntry->Addr, MAC_ADDR_LEN);
			/* end: add by chenzejun for mac scan by 2016.1.22 */
