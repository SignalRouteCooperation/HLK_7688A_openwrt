...
ol_txrx_peer_find_hash_attach(struct ol_txrx_pdev_t *pdev)
{
    ...
    /* allocate an array of TAILQ peer object lists */
    pdev->ast_entry_hash.bins = adf_os_mem_alloc(
            pdev->osdev,
...
#if PEER_FLOW_CONTROL
    adf_os_mem_free(pdev->ast_entry_hash.bins);
#endif
}
    ...
    struct ol_txrx_peer_t *peer)
{
     ...
     */
     ...
     if (!found) {
          adf_os_print("%s:Peer (%p) not found at hash index %d traversing complete "
...
#ifdef OL_TXRX_AST_DEBUG
    adf_os_print("%s ast_entry not found \n", __func__);
#endif
        ...
        u_int8_t *peer_mac_addr)
{
...
#ifdef OL_TXRX_AST_DEBUG
    adf_os_print("%s %p: peer mac %02x:%02x:%02x:%02x:%02x:%02x\n", __func__, pdev, mac_addr->raw[0],mac_addr->raw[1],mac_addr->raw[2],mac_addr->raw[3],mac_addr->raw[4],mac_addr->raw[5]);
#endif
    ...
    if (!ast_entry) {
        adf_os_print("%s ast entry delete failed \n", __func__);
        return -1;
    }
    ...
    u_int16_t peer_id)
{
    ...
    if ((peer->vdev) && (peer->vdev->pdev != pdev)) {
        adf_os_print("%s: Peer lookup returned invalid peer %p peer_id %d, pdev %p\n", __func__, peer, peer_id, pdev);
        return;
    }

    if(ol_txrx_ast_find_hash_remove(pdev, (u_int8_t *)&peer->mac_addr)) {
       union ol_txrx_align_mac_addr_t *mac_addr = (union ol_txrx_align_mac_addr_t *)&peer->mac_addr;
       adf_os_print("%s:Peer map has not handled properly for this, peer %p,peer_id %d,peer_mac %02x:%02x:%02x:%02x:%02x:%02x \n", __func__,peer,peer_id,mac_addr->raw[0],mac_addr->raw[1],mac_addr->raw[2],mac_addr->raw[3],mac_addr->raw[4],mac_addr->raw[5]);
       return;
    }
