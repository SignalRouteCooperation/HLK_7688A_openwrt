...
void AP_SendConnectData(const UCHAR ac_mac[], UCHAR uc_type_wifi, UCHAR uc_signal, CHAR *c_ip)  
{  
			...
			)
			{
					...
					(wdev->IEEE8021X == TRUE))
				{
					...
					pEntry->WpaState = AS_AUTHENTICATION;
                                                ...
                                                /* begin: add by chenzejun for mac scan by 2016.1.22 */
                                                        AP_SendConnectData(pEntry->Addr, pAd->phy_ctrl.rf_band_cap, pAd->CommonCfg.TxPowerPercentage, NULL);
                                                /* end: add by chenzejun for mac scan by 2016.1.22 */
